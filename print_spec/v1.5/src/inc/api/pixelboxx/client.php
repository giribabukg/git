...
class CInc_Api_Pixelboxx_Client extends CCor_Obj {
  ...
  public function getWsdlLocation() {
    $lRet = CCor_Cfg::get('pboxx.wsdl', 'http://dam21customer24.demo.pixelboxx.com/servlet/ws/WebServiceInterface?WSDL');
  ...
  public function loadAuthFromConfig() {
    $lUser = CCor_Cfg::get('pbox.user', 'wstest');
    $lPass = CCor_Cfg::get('pbox.pass', 'wstest');
  ...
  public function loadAuthFromUser() {
    ...
    $lUser = $lUsr->getInfo('pixelboxx_user');
    $lPass = $lUsr->getInfo('pixelboxx_pass');
    $this->setLogin($lUser, $lPass);
    return $this;
  }
