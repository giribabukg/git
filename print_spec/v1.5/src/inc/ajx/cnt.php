 ...
 * @version $Rev: 13977 $
 * @date $Date: 2016-05-19 23:43:04 +0800 (Thu, 19 May 2016) $
...
// 	}
  ...
  protected function actUsremail() {
    ...
    $lSql = 'SELECT firstname,lastname,email FROM al_usr';
    $lSql.= ' WHERE mand IN (0,'.MID.')';
    $lSql.= ' AND del="N"';
    $lSql.= ' AND email<>""';
    $lSql.= ' AND (firstname LIKE "'.$lVal.'%" OR lastname LIKE "'.$lVal.'%")';
    $lSql.= ' ORDER BY lastname';
    $lSql.= ' LIMIT 10;';
...
//		$lArr[] = 'cnfUnDel';
//		$lArr[] = 'createArt';
//		$lArr[] = 'createMasterVariant';
//		$lArr[] = 'jobDelFile';
//		$lArr[] = 'saveReport';
//		$lArr[] = 'jobWecUploadFile';
//		$lArr[] = 'ListCheck';
//		$lArr[] = 'cnfUnassign';
//
//		echo 'lan = new Array();';
...
//		}
//		foreach($lArr as $lVal){
//			echo 'lan["' . $lVal . '"] = new Array();';
//			foreach($lAvailLang as $lLang => $lName){
//				echo 'lan["' . $lVal . '"]["' . $lLang . '"] = "' . lang('js.' . $lVal, $lLang) . '";';
//			}
//		}
//		exit();
//	}
  ...
  protected function actUpdatethumbnail() {
    ...
    if ($lWecAvailable == TRUE && $lDalimAvailable == FALSE) {
      $lJobNr = ltrim($lJobId, '0');
      $lWecPrjId = CCor_Qry::getStr('SELECT wec_prj_id FROM al_job_shadow_'.MID.' WHERE jobid='.esc($lJobId));
      if (!$lWecPrjId) {
        $lWecPrjId = CCor_Qry::getStr('SELECT wec_prj_id FROM al_job_arc_'.MID.' WHERE jobid='.esc($lJobId));
        if (!$lWecPrjId) {
          $lStd = array('err' => lan('lib.wecprjid.not.found'));
      ...
      if (count($lWecDocRes) > 1) {
        ...
        $lWecVerID = $lWecDocRes[0]['wec_ver_id'];
        $lWecDocID = $lWecDocRes[0]['wec_doc_id'];
      }

      if (!$lWecDocID) {
        $lStd = array('err' => lan('lib.wecdocid.not.found'));
        echo json_encode($lStd);
        exit;
      }
      ...
      if ($lImage) {
        ...
        // create thumbnail
        $lThumbnail = @imagecreatefromjpeg($lPreviewImageFound.$lDynamics['image_file']);
        if ($lThumbnail) {
          $lImageWidth = imagesx($lThumbnail);
          $lImageHeight = imagesy($lThumbnail);

          $lThumbnailWidth = 100;
          $lThumbnailHeight = 100;

          $lSmallImage = imagecreatetruecolor($lThumbnailWidth, $lThumbnailHeight);
          imagecopyresampled($lSmallImage, $lThumbnail, 0, 0, 0, 0, $lThumbnailWidth, $lThumbnailHeight, $lImageWidth, $lImageHeight);
          imagejpeg($lSmallImage, $lThumbnailFound.$lDynamics['thumbnail_file'], 75);
        } else {
          $lStd = array('err' => lan('lib.thumbnail.not.created'), 'msg' => lan('lib.new.image.found'));
          echo json_encode($lStd);
          exit;
        }
      } else {
        $lStd = array('err' => lan('lib.image.not.created'), 'msg' => lan('lib.new.image.found'));
        echo json_encode($lStd);
        exit;
      }

      $lImg = $lDynamics['image_dir'].$lDynamics['image_file'];
      $lThb = $lDynamics['thumbnail_dir'].$lDynamics['thumbnail_file'];

      $lResult = CCor_Qry::getStr('SELECT 1 FROM al_job_wectns_'.MID.';');
      if (!$lResult) {
        $lResult = CCor_Qry::exec('CREATE TABLE `al_job_wectns_'.MID.'` (
          `mid` CHAR(4) NOT NULL,
          `src` CHAR(3) NOT NULL,
          `jobid` CHAR(13) NOT NULL,
          `jobnr` INT(11) NULL DEFAULT NULL,
          `wec_prj_id` CHAR(16) NOT NULL,
          `wec_doc_id` CHAR(16) NOT NULL,
          `wec_ver_id` CHAR(16) NULL DEFAULT NULL,
          `repository` CHAR(16) NULL DEFAULT NULL,
          `downloaded` DATETIME NOT NULL,
          `checked` DATETIME NULL DEFAULT NULL,
          `copied_to_portal` DATETIME NULL DEFAULT NULL,
          `copied_to_networker` DATETIME NULL DEFAULT NULL,
          UNIQUE INDEX `master_unique` (`jobid`, `wec_prj_id`, `wec_doc_id`)
          )'
        );
      }

      $lResult = CCor_Qry::exec('UPDATE al_job_wectns_'.MID.' SET wec_ver_id='.esc($lWecVerID).', downloaded=NOW() WHERE jobid='.esc($lJobId).' AND wec_prj_id='.esc($lWecPrjId).' AND wec_doc_id='.esc($lWecDocID).';');
      if (mysql_affected_rows() == 0) {
        $lResult = CCor_Qry::exec('INSERT INTO al_job_wectns_'.MID.' (mid,src,jobid,jobnr,wec_prj_id,wec_doc_id,wec_ver_id,repository,downloaded) VALUES ("'.MID.'", "'.$lSrc.'", "'.$lJobId.'", "'.$lJobNr.'", "'.$lWecPrjId.'", "'.$lWecDocID.'", "'.$lWecVerID.'", "alink", NOW());');
      }

      $lStd = array('img' => $lImg, 'thb' => $lThb, 'msg' => lan('lib.new.image.found'));
      echo json_encode($lStd);
      exit;
    } else {
      $lSvcWecInst = CSvc_Wec::getInstance();
