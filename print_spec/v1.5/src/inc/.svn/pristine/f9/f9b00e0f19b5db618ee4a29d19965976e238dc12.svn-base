<?php
class CInc_Gru_Rig_Form extends CCor_Ren {

  public function __construct($aGroupId, $lMandatorId, $lRightCode = '') {
    $this -> mGroupId = intval($aGroupId);
    $this -> mMandatorId = intval($lMandatorId);
    $this -> mRightCode = $lRightCode;
  ...
  protected function getCont() {
    ...
    $lRet.= '<input type="hidden" name="act" value="gru-rig.sedt" />'.LF;
    $lRet.= '<input type="hidden" name="id" value="'.$this -> mGroupId.'" />'.LF;
  ...
  protected function getLines() {
    ...
    foreach ($this -> mDef as $lDef) {
      if ($lDef['val'] != $lOld) {
        ...
        $lRet.= '<td class="th2 w16 ac" data-toggle="tooltip" data-tooltip-head="" data-tooltip-body="'.lan('lib.sel.all').'"></td>';
        $lRet.= '<td class="th2 w16 ac" data-toggle="tooltip" data-tooltip-head="" data-tooltip-body="'.lan('lib.desel.all').'"></td>';
        $lRet.= '<td class="th2 w16 ac" data-toggle="tooltip" data-tooltip-head="" data-tooltip-body="'.lan('lib.info').'"></td>';
        $lRet.= '</tr>';
      }
  ...
  protected function getRights() {
    ...
    $this -> mMem = array(); // array of parent groups

    $lSql = 'SELECT id, name, parent_id FROM al_gru WHERE id=';
    $lQry -> query($lSql.$this -> mGroupId);
    $lRow = $lQry -> getAssoc();
    $lPar = $lRow['parent_id'];
    while (0 != $lPar) {
    ...
    $this -> mRig = array();
    $lSql = 'SELECT code,level FROM al_gru_rig WHERE group_id='.$this -> mGroupId.' ';
