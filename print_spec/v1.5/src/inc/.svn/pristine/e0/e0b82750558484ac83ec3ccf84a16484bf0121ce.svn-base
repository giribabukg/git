<?php
class CInc_Job_Tra_Apl_Cnt extends CCor_Cnt {
  ...
  protected function actStd() {
    ...
    $lVie = new CJob_Tra_Tabs($lJid, 'apl');
    $lRet.= $lVie -> getContent();

    $lFrm = new CJob_Apl_List($this -> mSrc, $lJid);
    $lRet.= $lFrm -> getContent();

    #$lApl = new CApp_Apl_Loop($this -> mSrc, $lJid, 'apl');
    #$lApl -> createLoop('2008-07-01');

    $lFrm = new CJob_Tra_Apl_Form('job-tra-apl.sedt', $lJid, $lJob, $lPag);
    $lRet.= $lFrm -> getContent();
  ...
  protected function actSnewmail() {
    ...
    $lMod = new CJob_His_Mod($this -> mSrc, $lJid, htMail);
