...
class CInc_Usr_List extends CHtm_List {
  ...
  public function getMemberships() {
    ...
    if (!empty($lArr)) {
      $lQry = new CCor_Qry('SELECT m.uid,g.code,g.name FROM al_usr_mem m, al_gru g WHERE g.id=m.gid AND m.uid IN ('.implode(',', $lArr).') ORDER BY g.name');
      foreach ($lQry as $lRow) {
  ...
  protected function getSearchForm() {
    ...
    $lJs.= 'document.observe("dom:loaded",function() {$("lis_ser").focus(); $("lis_ser").select()});';
  ...
  protected function getTdDel() {
    ...
    if ('N' == $lVal) {
      ...
      $lRet.= '<a class="nav" href="javascript:Flow.Std.cnfUnDel(\''.$lUrl.'\', \''.LAN.'\')">';
    }
