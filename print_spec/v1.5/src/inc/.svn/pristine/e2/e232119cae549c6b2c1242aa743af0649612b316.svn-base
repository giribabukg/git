<?php
class CInc_Gru_Crp_Status_Form extends CCor_Ren {
  ...
  protected function getPriv() {
    ...
    $lSql = 'SELECT sta_id FROM al_gru_rig_status WHERE gru_id='.$this -> mGid.' AND sta_id IN ('.$lSid.')';
    $lQry -> query($lSql);
  ...
  protected function getCont() {
    ...
    $lRet.= '<input type="hidden" name="act" value="gru-crp-status.sedt" />'.LF;
