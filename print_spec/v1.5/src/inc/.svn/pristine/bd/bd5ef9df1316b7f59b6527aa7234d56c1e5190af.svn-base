<?php
class CInc_Job_Adm_Apl_Cnt extends CCor_Cnt {
  ...
  protected function actStd() {
    ...
    $lVie = new CJob_Adm_Tabs($lJid, 'apl');
    $lRet.= $lVie -> getContent();

    $lFrm = new CJob_Apl_List('adm', $lJid);
    $lRet.= $lFrm -> getContent();

    #$lApl = new CApp_Apl_Loop('adm', $lJid, 'apl');
    #$lApl -> createLoop('2008-07-01');

    $lFrm = new CJob_Adm_Apl_Form('job-adm-apl.sedt', $lJid, $lJob, $lPag);
    $lRet.= $lFrm -> getContent();
  ...
  protected function actNewmail() {
    ...
    $lVie = new CJob_Apl_Mailform('adm', $lJid, $lJob);
    $lRet.= $lVie -> getContent();
  ...
  protected function actSnewmail() {
    ...
    $lMod = new CJob_His_Mod('adm', $lJid, htMail);
   ...
   # $lMod -> insert(); //braucht nicht dokumentiert zu werden.
    ...
    $this -> redirect('index.php?act=job-adm-apl&jobid='.$lJid);
  }
