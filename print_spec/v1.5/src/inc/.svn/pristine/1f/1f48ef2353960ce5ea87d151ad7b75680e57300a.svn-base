  ...
  protected function getCont() {
    ...
    if ($lRow = $lQry -> getAssoc()) {
      ...
      foreach ($lQry as $lRow) {
        ...
        if ($lUsr -> canInsert('vie')) {
          $lRet.= '<a class="nav" href="javascript:Flow.Std.cnfDel(\'index.php?act=job-view.del&amp;src='.$this -> mSrc.'&amp;id='.$lId.'\', \''.LAN.'\')">';
    ...
    if ($lRow = $lQry -> getAssoc()) {
      ...
      foreach ($lQry as $lRow) {
        ...
        $lRet.= '</td>'.LF;
        $lRet.= '<td class="'.$lCls.' ac">';
        $lRet.= '<a class="nav" href="javascript:Flow.Std.cnfDel(\'index.php?act=job-view.del&amp;src='.$this -> mSrc.'&amp;id='.$lId.'&amp;jobid='.$this -> mJobId.'\', \''.LAN.'\')">';
        $lRet.= img('img/ico/16/del.gif');
        $lRet.= '</a>';
        $lRet.= '</td>'.LF;
        $lRet.= '</tr>'.LF;
