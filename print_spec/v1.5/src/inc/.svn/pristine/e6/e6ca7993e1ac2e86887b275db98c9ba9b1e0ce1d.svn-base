<?php
class CInc_Job_Com_Cnt extends CJob_Cnt {
  ...
  protected function actEdt() {
    ...
    $lVie = new CJob_Com_Tabs($lJobId, $lPag);
    $lRet.= $lVie -> getContent();

    $lFrm = new CJob_Com_Form('job-'.$this -> mSrc.'.sedt', $lJobId, $lJob, $lPag);
  ...
  protected function actSur() {
    ...
    $lVie = new CJob_Com_Tabs(0);
    $lRet = $lVie -> getContent();

    $lFrm = new CJob_Com_Form('job-com.ssur', 0, $lSKU);
  ...
  protected function actSsur() {
    ...
    if ($lMod -> insert()) {
      ...
      $this -> redirect('index.php?act=job-com.edt&jobid='.$lJobID.'&page='.$lPage);
    }
    $this -> redirect('index.php?act=job-com.new');
  }
  ...
  protected function actNsub() {
    ...
    $lVie = new CJob_Com_Tabs();
    $lRet.= $lVie -> getContent();

    $lFrm = new CJob_Com_Form('job-'.$this -> mSrc.'.snsub', 0, $lJob);
