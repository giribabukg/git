<?php
class CInc_Job_Sku_Sub_Item_List extends CHtm_List {

  public function __construct($aJobId) {
    parent::__construct('job-sku-sub');
    ...
    // job Kategories werden aus config geladen.
    //$this -> addColumn('job_art', lan('job-art.menu'), FALSE, array('width' => '70'));
    ...
    if ($this -> mCanInsArt) {
      $this -> addBtn(lan('job-sku-sub.createbundle'), 'createArt("'.$this -> mJobId.'","'.LAN.'")', 'img/ico/16/plus.gif');
    }
    $this -> addBtn(lan('lib.opt.fpr'), 'go(\'index.php?act='.$this -> mMod.'.fpr&jobid='.$this -> mJobId.'\')', 'img/ico/16/col.gif');
    if ($this -> mCanInsert) {
      $this -> addBtn(lan('job-sku-sub.new'), 'go(\'index.php?act='.$this -> mMod.'.new&jobid='.$this -> mJobId.'\')', 'img/ico/16/edit.gif');
    }
  ...
  protected function getSubMenu() {
    ...
    foreach ($this -> mWiz as $lKey => $lVal) {
      $lMen -> addItem('index.php?act=job-sku-sub.new&amp;jobid='.$this -> mJobId.'&amp;wiz='.$lKey, $lVal);
    }
  ...
  protected function getWizMenu() {
    ...
    $lMen -> addTh2(lan('lib.tpl'));
    foreach ($this -> mWiz as $lKey => $lVal) {
      $lMen -> addItem('index.php?act=job-sku-sub.wiz&amp;jobid='.$this -> mJobId.'&amp;wiz='.$lKey, $lVal);
    }
    return $lMen -> getContent();
  }
  ...
  protected function getIterator() {
    ...
    $this -> mIte -> addCnd('sku_id='.$this -> mJobId);
  }
  ...
  protected function addColumns() {
    $lUsr = CCor_Usr::getInstance();
    $lCol = $lUsr -> getPref('job-sku-sub.cols');
  ...
  protected function getTdIs_master() {
    ...
    $lSid = $this -> getInt('id');
    if ('Y' == $lMas) {
      $lRet = '<a href="index.php?act=job-sku-sub.master_unset&amp;jobid='.$this -> mJobId.'&amp;sid='.$lSid.'" class="nav">';
      $lRet.= img('img/ico/16/master.gif');
      $lRet.= '</a>';
    } else {
      $lRet = '<a href="index.php?act=job-sku-sub.master_set&amp;jobid='.$this -> mJobId.'&amp;sid='.$lSid.'" class="nav">';
  ...
  protected function getTdMenu($aSrc, $aJobID, $aFrom, $aCaption) {
    ...
    if( $this -> mArrLeerAmount[$aSrc] > 0 AND $this -> mCanSlide ){
      ...
      foreach ($this -> mArrLeer[$aSrc] as $lKey => $lVal) {
        $lMen -> addItem('index.php?act=job-sku-sub.slide&amp;jobid='.$aJobID.'&amp;from='.$aFrom.'&amp;to='.$lVal, 'Zeile '.$lKey);
      }
    }
    return $lMen -> getContent();
  }
...
//      $lRet.= '</a>';
  ...
  protected function getTdCpy() {
    $lSid = $this -> getInt('id');
    $lRet = '<a href="index.php?act=job-sku-sub.cpy&amp;jobid='.$this -> mJobId.'&amp;id='.$lSid.'" class="nav">';
