<?php
class CInc_Arc_Tra_Form extends CArc_Form {
  ...
  public function __construct($aAct, $aJobId = 0, $aJob = NULL, $aPage = 'job') {
    ...
    parent::__construct('tra', $aAct, $aPage, $this -> mJobId);
    
    $this -> mFla = 0;
    if (empty($aJob)) {
      if (!empty($this -> mJobId)) {
        $this -> mJob = new CJob_Tra_Dat();
    ...
    $this -> addBtn('act', lan('lib.cancel'), 'go("index.php?act=arc-tra")', 'img/ico/16/cancel.gif', 'button', array('class' => 'btn w200' ));

    $this -> mTabs = new CArc_Tra_Tabs($this -> mJobId, $aPage);
