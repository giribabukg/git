<?php
class CInc_Usr_Crp_Step_Form extends CCor_Ren {
  ...
  protected function getCheck($aStp, $aFlg = 0) {
    $lRet = '';
    $lSid = $aStp['id'];
    if (isset($this -> mMemRig[$lSid][$aFlg])) {
      $lMem = implode('<br />', $this -> mMemRig[$lSid][$aFlg]);
  ...
  protected function getPriv() {
    ...
    $lQry = new CCor_Qry();
    $lSql = 'SELECT m.gid,g.name FROM al_usr_mem m, al_gru g WHERE m.gid=g.id AND m.uid=';
    $lQry -> query($lSql.$this -> mUid);
    ...
    $lSql = 'SELECT stp_id,fla_id FROM al_usr_rig_stp WHERE usr_id='.$this -> mUid.' AND stp_id IN ('.$lSid.')';
    $lQry -> query($lSql);
    foreach ($lQry as $lRow) {
    ...
    if (!empty($this -> mMem)) {
      $lSql = 'SELECT * FROM al_gru_rig_stp WHERE gru_id IN ('.implode(',', array_keys($this -> mMem)).') ';
      $lSql.= 'AND stp_id IN ('.$lSid.')';
      $lQry -> query($lSql);
      foreach ($lQry as $lRow) {
        $this -> mMemRig[ $lRow['stp_id'] ][ $lRow['fla_id'] ][] = $this -> mMem[$lRow['gru_id']];
      }
    }
  }
  ...
  protected function getCont() {
    ...
    $lRet.= '<input type="hidden" name="act" value="usr-crp-step.sedt" />'.LF;
