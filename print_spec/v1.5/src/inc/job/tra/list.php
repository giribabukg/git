<?php
class CInc_Job_Tra_List extends CJob_List {

  protected $mAva = fsTra;
  protected $mSrc = 'tra';
  ...
  public function __construct($aWithoutLimit = FALSE, $aAnyUsrID = NULL) {
    ...
    if ($lUsr -> canInsert('job-'.$this -> mSrc)) {
      $this -> addBtn(lan('job-'.$this -> mSrc.'.new'), 'go("index.php?act=job-'.$this -> mSrc.'.new")', 'img/ico/16/plus.gif');
    }
