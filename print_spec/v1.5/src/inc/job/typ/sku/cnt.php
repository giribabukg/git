<?php
class CInc_Job_Typ_Sku_Cnt extends CCor_Cnt {
  ...
  public function actStd() {
    ...
    $lJob = new CJob_Typ_Dat();
    $lJob -> load($lJobID);

    $lRet = '';

    $lVie = new CJob_Typ_Header($lJob);
    $lRet.= $lVie -> getContent();

    $lVie = new CJob_Typ_Tabs($lJobID, 'sku');
    $lRet.= $lVie -> getContent();

    $lVie = new CJob_Typ_Sku_List($lJobID);
    $lRet.= $lVie -> getContent();
  ...
  protected function actUnassign() {
    ...
    if (!empty($lJobID) AND !empty($lSKUID)) {
      $lSql = 'DELETE FROM al_job_sku_sub_'.intval(MID).' WHERE sku_id='.esc($lSKUID).' AND job_id='.esc($lJobID);
      CCor_Qry::exec($lSql);
    }
