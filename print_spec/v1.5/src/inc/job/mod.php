 ...
 * @version $Rev: 12967 $
 * @date $Date: 2016-03-15 18:27:56 +0800 (Tue, 15 Mar 2016) $
    ...
    foreach ($this -> mFie as $lKey => $lDef) {
      ...
      if (('string' == $lTyp) or ('pick' == $lTyp)) {
        if (($aNew) or ($this -> fieldHasChanged($lKey))) {
          ...
          if($lKey == 'client_key' && !empty($lVal)) {
            $lCmsMod = new CJob_Cms_Mod($this -> mJobId, $this -> mSrc);
            $lCmsMod -> updateProductRefs($this -> mJobId, $this -> mSrc, $lVal);
          }
        }
      }
    }
