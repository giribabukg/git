<?php
class CInc_Job_Sku_Sub_Cnt extends CCor_Cnt {

  public function __construct(ICor_Req $aReq, $aMod, $aAct) {
    parent::__construct($aReq, $aMod, $aAct);

    $this -> mAva = fsSku;
    $this -> mJobID = $this -> mReq -> jobid;
  ...
  protected function actStd() {
    ...
    $lVie = new CJob_Sku_Tabs($lSKUID, 'sub');
    $lRet.= $lVie -> getContent();

    $lVie = new CJob_Sku_Sub_List($lSKUID);
    $lRet.= $lVie -> getContent();
  ...
  protected function actDel() {
    ...
    $lSql = 'DELETE FROM al_job_sub_'.intval(MID).' WHERE id='.esc($lId);
    CCor_Qry::exec($lSql);
