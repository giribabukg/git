 ...
 * @version    $Rev: 10713 $
 * @date       $Date: 2015-10-06 16:52:39 +0800 (Tue, 06 Oct 2015) $
...
//     error_log('.....$lSys at sys class.....'.var_export($lSys,true)."\n",3,'logggg.txt');
  ...
  public static function insertJobFile($lSrc, $lJid, $lSub, $lFil, $lCat = '') {
      $lSql = 'INSERT INTO al_job_files SET src='.esc($lSrc).',jobid='.esc($lJid).',';
      $lSql.= 'uid='.CCor_Usr::getAuthId().',';
      $lSql.= 'sub='.esc($lSub).',';
      if (isset($lCat))
          $lSql.= 'category='.esc($lCat).',';
  ...
  /**
   * Get all groups (except the root groups) the user is a member of
   * @return string Comma separated list of group IDs
   */
  public static function getMembershipImplode() {
    $lUid = self::getAuthId();
    $lSql = 'SELECT p.id FROM al_gru p, al_usr_mem q WHERE p.id=q.gid AND p.mand='.MID.' AND p.parent_id<>0 AND q.uid='.$lUid;
