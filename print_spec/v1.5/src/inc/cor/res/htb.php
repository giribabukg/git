<?php
class CInc_Cor_Res_Htb extends CCor_Res_Plugin {
  ...
  protected function refresh($aParam = NULL) {
    ...
    $lCkey = 'cor_res_htb_'.$aParam.'_'.LAN;
    if ($lRet = $this -> getCache($lCkey)) {
      return $lRet;
    }

    $lRet = array();
    $lSql = 'SELECT '.$lFirst.', value_'.LAN.' FROM al_htb_itm WHERE mand IN(0,'.MID.') AND domain='.esc($lDomain).' ORDER BY '.$lSort;
