		...
		<table class="table table-striped table-hover table-condensed">
			<tbody>
				<tr>
					...
					<td class="col-sm-4 col-md-6 col-lg-7">{$barcodes[0]['ZLP_BC_SUB_TYPE']}</td>
				</tr>
				<tr>
					<td><strong>{$text->__('ZLP_BC_CODE_COLOR')}</strong></td>
					<td>{$barcodes[0]['ZLP_BC_CODE_COLOR']}</td>
				</tr>
				
				<tr>
					<td><strong>{$text->__('ZLP_BC_BCKGRND_COLOR')}</strong></td>
					<td>{$barcodes[0]['ZLP_BC_BCKGRND_COLOR']}</td>
				</tr>
				<tr>
					<td><strong>{$text->__('ZLP_BC_TEST_PROTOCOL')}</strong></td>
					<td>{$barcodes[0]['ZLP_BC_TEST_PROTOCOL']}</td>
				</tr>
				<tr>
					<td><strong>{$text->__('ZLP_BC_CODE_SIZE')}</strong></td>
					<td>{$barcodes[0]['ZLP_BC_CODE_SIZE']}</td>
				</tr>
				<tr>
					<td colspan="2">{$barcodes[0]['ZLP_BC_MEMO_TXT']|nl2br}</td>
				</tr>
			</tbody>
		</table>
	</div>
