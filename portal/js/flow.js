Flow = {
	chkFileSize : function(){
		var lCategory = jQuery('select[name="category"]').val();
		lCategory = jQuery('select[name="category"] option[value="'+lCategory+'"]').text();
		var lFilename = jQuery("input[name='file']")[0].files[0].name;
		var lSize = jQuery("input[name='file']")[0].files[0].size;
		var lParam = {
			'act' : 'job-fil.checkfilesize',
			'size' : lSize,
			'ext': lFilename.split('.').pop().toLowerCase(),
			'category' : lCategory
		};
		jQuery.post('index.php', lParam, function(aResponse) {
            if(aResponse == 'false') {
            	alert('File uploaded exceeds maximum upload size or invalid file type!');
            	return false;
            }
        });
	},
	checkPwd : function(){
		var lVal = {
			'old': jQuery('input[name="val[old]"]').val(),
			'new': jQuery('input[name="val[new]"]').val(),
			'cnf': jQuery('input[name="val[cnf]"]').val()
		}
		var lParam = { 'act' : 'hom-pwd.check', 'val' : lVal };
		jQuery.post('index.php', lParam, function(aResponse) {
            if(aResponse !== '') {
            	alert(aResponse);
            } else {
	...
	checkUsrDet : function(){
		var lVal = {
			'usr': jQuery('input[name="val[user]"]').val(),
			'pass': jQuery('input[name="val[pass]"]').val()
		}
		var lParam = { 'act' : 'hom-usr.check', 'val' : lVal };
		jQuery.post('index.php', lParam, function(aResponse) {
            if(aResponse !== '') {
            	alert(aResponse);
            } else {
	...
	checkUsrAvail : function(){
		var lVal = {
			'usr': jQuery('input[name="val[to_emails]"]').val()
		}
		var lParam = { 'act' : 'gru.checkUsrAvail', 'val' : lVal };
		jQuery.post('index.php', lParam, function(aResponse) {
            if(aResponse !== '') {
            	alert(aResponse);
            } else {
                ...
                onSuccess : function(transport) {
                    if('' != transport.responseText) {
                        Element.clonePosition(lDiv, select, {
                            setHeight : false,
                            setWidth : false,
                            offsetLeft : jQuery(select).width()
                        });
                        lDiv.show();
                    }
                }
    ...
    crpTip : function(aEl, aId, aOff) {
        ...
        if (lLeft > lDivX.width()) {
            var lOff = jQuery(aEl).width() - lDivX.width() - 1;
            Element.clonePosition(lDiv, aEl, {
                setHeight : false,
                setWidth : false,
                offsetTop : jQuery(aEl).height(),
    ...
    togTrTree : function(aElement, aClass) {
        ...
        if(lTr.hasClass('collapsed')) {
            ...
            jQuery('.' + aClass).each(function() {
                Flow.collapseAllTree(jQuery(this).find('.mor').find('a'), jQuery(this).attr('id'));
                jQuery(this).css('display', 'none');
                jQuery(this).addClass('collapsed');
            });
        }
    },
    loadTrTree : function(aElement, aID, aClass, aLvl, aStart) {
      var lBlockSize = 4000;
      var lTr = jQuery(aElement).closest('tr');
      var lMor = jQuery(lTr).find('.mor');
      //Set loading gif
      if(aStart === 0) {
        lMor.find('a').hide();
        lMor.append('<img src="img/pag/ajx.gif" class="loader'+aClass+'" alt="" title="" width="16" height="16">');
      }
      //Setup Ajax
      var ajx = jQuery.ajax({
        beforeSend: function() {
        },
        url:'index.php?act=ajx.getChildGroups',
        method:'POST',
        dataType: 'json',
        data: {
          pid : aID,
      ...
      //Run if Success
      ajx.success(function(lRes) {
        if(lRes[0] === 'success') {
          //Find Target tr for inserting new rows
          var lCnt = (jQuery(lRes[1]).size() / 2) + 0.5;
          if(jQuery('.'+aClass).last().size() > 0) {
            lTarget = jQuery('.'+aClass).last();
          }
          else {
            lTarget = lTr;
          }
          jQuery(lTarget).after(lRes[1]).hide().slideDown();
          //Check if more rows should be loaded
          if(lCnt === lBlockSize) {
            Flow.loadTrTree(aElement, aID, aClass, aLvl, aStart + lBlockSize);
          }
          else {
            //Finished with loading and reset loading gif with toggle button
            jQuery('.loader'+aClass).fadeOut().remove();
            var lMor = lTr.find('.mor').find('a');
            lMor.fadeIn();
            lMor.attr('onclick','Flow.togTrTree(this, "'+aClass+'")');
          }
        }
      });
      return ajx;
    },
    expandAllTree : function(aElement, aClass, aLevel) {
      jQuery("a[onclick*='load']").each(function() {
        var lTr = jQuery(this).closest('tr[id^="pr"]');
        var lId = lTr.attr('id');
        var lMor = lTr.find('.mor').find('a');
        lMor.attr('onclick','Flow.togTrTree(this, "'+lId+'")');
        jQuery.when(Flow.loadTrTree(this,lId.substring(2),lId,aLevel,0)).done(function() {
          Flow.expandAllTree(aElement, aClass, aLevel +1);
        });
      });
...
Flow.apl = {
    getOverview : function(aEventId, aJid, aPrefix, aContainer, aCanDeselect) {
...
//        });
        ...
        jQuery('#apl-dlg').dialog({
            ...
            buttons : {
                Okay : function() {
                    ...
                    Flow.apl.postStatusAndReload(aId, aSubId, aFlag, jQuery('#apl-comment').val());
                },
                'Cancel' : function() {
                    jQuery('#apl-dlg').dialog('close');
                }
    ...
    postStatusAndReload : function (aId, aSubId, aFlag, aComment) {
        var lParams = { id: aId, flag : aFlag, comment : aComment};
...
Flow.thumbnail = {
    ...
    update : function(aParams) {
        jQuery.ajax({
            ...
            if (lImg) {
                jQuery('a[id="a' + aParams.jobid + '"]').attr('data-tooltip-body', "<img src='data:image/jpeg;base64," + lImg + "' border='0p' width='300' height='300px' alt='' />");
            }

            if (lThb) {
                jQuery('img[id="img' + aParams.jobid + '"]').attr('src', 'data:image/jpeg;base64,' + lThb);
            }
        });
    }
};
...
Flow.proofscope = {
    compare : function(aSrc, aJid) {
        var sel = new Array();
        jQuery('.beh-comp:checked').each(function() {
            sel.push(jQuery(this).val());
        });
        lParams = {};
        lParams.file = sel[0];
        ...
        jQuery.post('index.php?act=utl-fil.viewproofscopediff', lParams, function(aResponse) {
            if (aResponse.status == 'ok') {
                var lUrl = aResponse.url;
...
Flow.job = {
    ...
    saveJobQuestion : function(aId, aState, aSrc, aJobId, aQuestText) {
      var lQuest = jQuery('.quest_'+aId);
      var lMsg = lQuest.find('textarea');
      var lChg = lQuest.find('.lastChanged');
      var lIco = lQuest.find('.stateIco');
      var lSBtn = lQuest.find('.saveButtons');

      if(!lMsg.val().trim()) {
        alert (lan('questions.emptyNotSaved'));
        return;
      }

      //Send Ajax Call for saving the Answer
      var ajx = jQuery.ajax({
        beforeSend: function() {
          lIco.removeClass('ico-w16-flag-01');
          lIco.removeClass('ico-w16-flag-02');
          lIco.removeClass('ico-w16-flag-03');
          lIco.removeClass('ico-w16-ml-1');
          lIco.addClass('ico-w16-clock_refresh');
        },
        url:'index.php?act=ajx.saveJobQuestion',
        method:'POST',
        dataType: 'json',
        data: { id : aId,
                msg: lMsg.val(),
      ...
      ajx.success(function(lRes) {
        if(lRes[0] === 'success') {
          lIco.removeClass('ico-w16-ml-1');
          lMsg.css('border', '1px solid #CCC');
          if(lRes[3] === '3') {
            lMsg.attr('readonly', true);
            lMsg.addClass('dis');
          }
          else {
            lMsg.removeAttr('readonly');
            lMsg.removeAttr('disabled');
            lMsg.removeClass('dis');
          }
        }
        else {
          lIco.addClass('ico-w16-ml-1');
        }
        lChg.fadeTo('fast', 0).queue(function() { lChg.html(lRes[1]); lChg.dequeue(); }).fadeTo('fast',1);
        lSBtn.fadeTo('fast', 0).queue(function() { lSBtn.html(lRes[4]); lSBtn.dequeue(); }).fadeTo('fast',1);
        lIco.fadeTo('fast', 0).queue(function() { lIco.removeClass('ico-w16-clock_refresh'); lIco.addClass(lRes[2]); lIco.dequeue(); }).fadeTo('fast',1);
      });
      ajx.fail(function() {
        alert(lan('questions.notSaved'));
      });
    },
    filterQuestions : function(aStatus) {
      lQuests = jQuery("div[class*=\'quest_\']");
      jQuery.each(lQuests, function(lKey, lVal) {
        if(aStatus === -1 || jQuery(lVal).find('.stateIco').hasClass('ico-w16-flag-0'+aStatus)) {
          jQuery(lVal).slideDown();
        }
        else {
          jQuery(lVal).slideUp();
        }
    ...
    saveAllQuestions : function(aState, aSrc, aJobId) {
      var lCheckedQ = jQuery("input[name*=\'QL_check_\']:checked");
      if(lCheckedQ.length > 0) {
        jQuery.each(lCheckedQ, function(lKey, lVal) {
          var lId = jQuery(lVal).attr('name').substr(9);
          var lQuestName = jQuery("div[class*=\'quest_"+lId+"\']").find("strong").text();

          Flow.job.saveJobQuestion(lId, aState, aSrc, aJobId, lQuestName);
        });
      }
      else {
        alert(lan('questions.zeroSelected'));
      }
    },
...
Flow.Std = {
    ...
    // **************************************************
    fprSel : function(aSrc, aDest, maxNum) {

        var lSrc = getElem(aSrc);
        var lDst = getElem(aDest);
        for(var i = 0; i < lSrc.options.length; i++) {
            if(lSrc.options[i].selected) {
              if(maxNum > 0 && (jQuery('#'+aDest+' option').size() >= maxNum) && aDest == "dst") {
                alert("More Users than allowed for this group!");
                break;
              }
    ...
    cnf: function(aUrl, aLan) {
        if(confirm(unescape(lan('js.' + aLan)))) {
            location.href = aUrl;
        }
    },
    // **************************************************
    ...
    getElem : function(aId) {
        return (typeof aId == 'string') ? jQuery('#' + aId)[0] : aId;
    },
    // **************************************************
    // TOGGLE
    // **************************************************
    ...
    togCpl : function(aDivId, aImgId, aLnkId, aPersist) {
        try {
            ...
            aLnkId = aLnkId.replace('.', '\\.');
            aImgId = aImgId.replace('.', '\\.');
    ...
    ajxUpd : function(aParams) {
        if (aParams.loading_screen && aParams.div) {
    ...
    chkDom : function(aEl, aImg, aBlackList) {
      var lChk = 0;
      var lVal = aEl.value.split(',');
      var lLength = jQuery(aEl).val().length;
      var lRegEx = new RegExp('^[A-Za-z0-9_.-]+[.][a-zA-z]{2,6}$'); //searches for Domains like: 5flow.com
      for(var i = 0;i< lVal.length; i++) {
        lVal[i].slice(lVal[i].indexOf('@'),lVal[i].length);
        lVal[i] = aEl.value.trim().replace(/.*@/, "");
        if(!lRegEx.test(lVal[i]) || aBlackList.indexOf(lVal[i]) !== -1) {
          lChk = 1;
        }
      }

      if(lChk === 0 || lLength === 0) {
        $(aImg).src = 'img/wave8/ico/16/ok.gif';
        jQuery(":submit").removeAttr("disabled");
      } else {
        $(aImg).src = 'img/wave8/ico/16/ml-4.gif';
    ...
    tMce : function(aEl) {
        ...
        tinyMCE.init({
            ...
            setup: function(ed) {
                if(jQuery('#'+ed.id).attr('readonly'))
                    ed.settings.readonly = true;

                if(jQuery('#'+ed.id).attr('onkeyup')) {
                    ed.onKeyUp.add(function() {
                    	var keyup = jQuery('#'+ed.id).attr('onkeyup');
                    	keyup = keyup.replace("javascript:", '').split("(");
                    	var func = keyup[0];
                    	var args = keyup[1].replace(");", '').split(", ");
...
Flow.FieldMap =
{
    ...
    loadSub : function (aId) {
        var td = jQuery('#m' + aId + 'r');
        td.html('<img src="img/pag/ajx.gif">').load('index.php?act=fie-map.sub&id=' +aId);
    },

    restoreSearch : function (aTerm) {
        if (aTerm != '') {
            var el = td.find('.term');
            el.val(aTerm);
            Flow.FieldMap.search(td.find('.term'));
        }
    },

    submitDialog : function (aDiv, aMapId, aUrl) {
        var lVal = {};
        var fields = ['alias','native','default_value','validate_rule','read_filter','write_filter'];
        for (var i = 0; i < fields.length; i++) {
            var el = jQuery('.field_' + fields[i], aDiv);
            if (el.length) {
                lVal[fields[i]] = el.val();
            }
        }
        var lParams = {mid : aMapId, val : lVal};
        var td = jQuery('#m' + aMapId + 'r');
        var term = td.find('.term').val();
        td.load(aUrl, lParams, function() {
            var inp = td.find('.term');
            inp.val(term);
            Flow.FieldMap.search(inp);
        });
    },
    ...
    deleteItem : function (aMapId, aId) {
        var td = jQuery('#m' + aMapId + 'r');
        var term = td.find('.term').val();
        td.load('index.php?act=fie-map.delitem&fid=' +aId +'&mid='+aMapId, function() {
            var inp = td.find('.term');
            inp.val(term);
            Flow.FieldMap.search(inp);
        });
    ...
    newItem : function(aMapId) {
        var div = jQuery('<div />');
        jQuery(div).load('index.php?act=fie-map.newitem&map='+aMapId, function() {
            jQuery(div).dialog({
                title : 'Add New Field',
                width : '450px',
                modal: true,
                ...
                buttons: {
                    Okay : function() {
                        jQuery(this).dialog('close');
                        Flow.FieldMap.submitDialog(div, aMapId, 'index.php?act=fie-map.snewitem');
                    },
                    Cancel : function() {
                        jQuery(this).dialog('close');
                    }
        ...
        jQuery(div).load('index.php?act=fie-map.edititem&fid='+aId, function() {
            jQuery(div).dialog({
                ...
                buttons: {
                    Okay : function() {
                        jQuery(this).dialog('close');
                        Flow.FieldMap.submitDialog(div, aMapId, 'index.php?act=fie-map.sedititem&fid='+aId);
                    },
                    Cancel : function() {
                        jQuery(this).dialog('close');
                    }
    ...
    addJobFields : function(aMapId, aIsCore) {
        var div = jQuery('<div />');
        div.load('index.php?act=fie-map.addjobfields&mid='+aMapId+'&core='+aIsCore, function() {
            jQuery('table', div).attr('unselectable','on')
            ...
            div.dialog({
                ...
                open : function() {jQuery('tr.val').click(Flow.FieldMap.togSelect);},
                width : '850px',
                modal: true,
                resizable: false,
                buttons: {
                    Okay : function() {
                        jQuery(this).dialog('close');
                        var lVal = [];
                        jQuery('.bc-cb:checked', div).each(function() {
                            lVal.push(jQuery(this).val());
                        });
                        var lParams = {mid : aMapId, val : lVal, core : aIsCore};
    ...
    sendMap : function(aMapId) {
        var url = 'index.php?act=fie-map.send&mid='+aMapId;
        jQuery.post(url, function(aResponse) { alert(aResponse); });
    },

    exportMap : function(aMapId) {
        var url = 'index.php?act=fie-map.export&mid='+aMapId;
        location.href = url;
    },

    importMap : function() {
        var div = jQuery('<div><b>JSON</b><br /><textarea class="json" style="width:400px" rows="10"></textarea></div>');
        div.dialog({
            title : 'Import Field Map',
            modal: true,
            width : '450px',
            resizable: false,
            buttons: {
                Okay : function() {
                    jQuery(this).dialog('close');
                    var lJson = jQuery('.json', this).val();
                    var lParams = {json: lJson};
                    jQuery.post('index.php?act=fie-map.import', lParams, function(aResponse) {
                        if ('ok' == aResponse) {
                            location.href = 'index.php?act=fie-map';
                            return;
                        }
                        alert(aResponse);
                    });
    ...
    search : function(aElem, aPersist) {
        var $elem = jQuery(aElem);
        var parent = $elem.closest('.map-parent');
        var termRaw = jQuery('.term', parent).val();
        var term = termRaw.toLowerCase();
        var lines = jQuery('tr.val', parent);
        lines.show();
        if (term !== "") {
            $elem.disabled = true;
            lines.each(function () {
            ...
            jQuery(aElem).disabled = false;
        }
        if (aPersist) {
            var lParam = {key : aPersist, val : termRaw };
            jQuery.post('index.php?act=ajx.prf', lParam);
        }
    },

    togSearch : function(aElem) {
        var $elem = jQuery(aElem);
        var parent = $elem.closest('.map-parent');
        var lines = jQuery('tr.val', parent);
        lines.removeClass('dn');
        var cur = $elem.prop('data-cur');
        if ('undefined' == typeof cur) {
            cur = 0;
        }
       // console.log('old', lines);
        var newCur = 0;
        if (0 == cur) {
            jQuery('.bc-tg1', parent).addClass('dn');
            jQuery('.bc-togcap').text('with native');
            newCur = 1;
        } else if (1 == cur) {
            jQuery('.bc-tg2', parent).addClass('dn');
            jQuery('.bc-togcap').text('without native');
            newCur = 2;
        } else {
            jQuery('.bc-togcap').text('all');
        }
        $elem.prop('data-cur', newCur);
    },

    clearSearch : function(aElem) {
        var $elem = jQuery(aElem).closest('.map-parent').find('.term').val('');
        Flow.FieldMap.search(aElem);
    },

    initSearch : function(aElem) {
        var $elem = jQuery(aElem);
        var parent = $elem.closest('.map-parent');
        var lines = jQuery('tr.val', parent);
    ...
    togSelect : function (aEvent) {
        var $target = jQuery(aEvent.target);
        if ($target.hasClass('bc-cb')) {
            return;
        }
        var $tr = $target.closest('tr.val');
        var $allTr = jQuery('tr.val:visible');

        var cb = $tr.find('.bc-cb');
        if (!Flow.FieldMap.lastChecked) {
            Flow.FieldMap.lastChecked = $tr;
            cb.prop('checked', !cb.prop('checked'));
            return;
        }

        if (aEvent.shiftKey) {
            var start = $allTr.index(this);
            var end = $allTr.index(Flow.FieldMap.lastChecked);
            var $range = $allTr.slice(Math.min(start, end), Math.max(start, end) + 1);
            $range.find('.bc-cb').prop('checked', !cb.prop('checked'));
        } else {
            cb.prop('checked', !cb.prop('checked'));
        }
        Flow.FieldMap.lastChecked = $tr;
...
Flow.FieldForm = {
    ...
    initForm : function() {
        jQuery('.field_validate_rule_copy').val(jQuery('.field_validate_rule').val());
        Flow.FieldForm.setValidationFrom(jQuery('.field_native_core'));
    },

    setValidationFrom : function(aElem) {
        var lAlias = jQuery(aElem).val();
        var url = 'index.php?act=ajx.getvalidationfor';
        var params = { alias : lAlias };
        jQuery.post(url, params, function(aResponse) {
            var val = aResponse;
            if (val >0) {
                jQuery('.field_validate_rule').val(val);
                jQuery('.field_validate_rule_copy').val(val).prop('disabled', true).addClass('cl');
            } else {
                jQuery('.field_validate_rule_copy').prop('disabled', false).removeClass('cl');
            }
        });
    },

    copyValidateRule: function() {
        jQuery('.field_validate_rule').val(jQuery('.field_validate_rule_copy').val());
    },

    nativeSearch : function() {
        var div = jQuery('<div />');
        div.load('index.php?act=fie.nativesearch', function() {
            var elem = jQuery('button', div);
            Flow.FieldMap.search(elem);
            var that =
            jQuery('tr.val', div).click(function() {
                var native = jQuery(this).text();
                var field = jQuery('.field_native_core');
                field.val(native).trigger('change');
                jQuery(div).dialog('close');
            });
            div.dialog({
                title : 'Search for Native',
                modal: true,
                width : '290px',
                resizable: false,
                buttons: {
...
Flow.Files = function() {
    ...
    var gOldDirectorySelected;
    var gNewDirectorySelected;

    var gRelativeDirectory = '';
    var gDirectoryName = '';
    ...
    // TODO this should be universal
    this.validDirectoryname = function(aDirectoryName) {
        var lInvalidCharacters = new RegExp('[<>:"/\\|?*\x00-\x1F]', 'g');
        return aDirectoryName.match(lInvalidCharacters) ? false : true;
    };

    // TODO this should be universal
    this.isEmpty = function(aString) {
        return !aString;
    };

    this.clickDirectory = function(aThis) {
        jQuery('#button\\.directory\\.rename').prop('disabled', false);
        jQuery('#button\\.directory\\.remove').prop('disabled', false);

        if (gOldDirectorySelected == gNewDirectorySelected) {
            gNewDirectorySelected = jQuery(aThis);
            jQuery(gNewDirectorySelected).removeClass('nav').addClass('navhovered');

//            console.log(gNewDirectorySelected.data('relativedirectory'));
//            console.log(gNewDirectorySelected.data('directoryname'));

            if (isNaN(gNewDirectorySelected.data('relativedirectory'))) {
                gRelativeDirectory = '';
            } else {
                gRelativeDirectory = gNewDirectorySelected.data('relativedirectory');
            }

            if (isNaN(gNewDirectorySelected.data('directoryname'))) {
                gDirectoryName = '';
            } else {
                gDirectoryName = gNewDirectorySelected.data('directoryname');
            }

            var lParams = {
                'act': 'hom-fil.getfiles',
                'div': 'files',
                'relativedirectory': gRelativeDirectory,
                'directoryname': gDirectoryName,
                'loading_screen': true
            };
            Flow.Std.ajxUpd(lParams);
        } else {
            gOldDirectorySelected = gNewDirectorySelected;
            jQuery(gOldDirectorySelected).addClass('nav').removeClass('navhovered');
...
//            console.log(gNewDirectorySelected.data('directoryname'));
    ...
    this.createDirectory = function() {
        var lPath = gRelativeDirectory + gDirectoryName;
        var lName = jQuery('#directory\\.create').val();

//        console.log(lPath);
//        console.log(lName);

        if (gThis.isEmpty(lName)) {
            jQuery('#directory\\.create\\.label').css('color', 'red').text(lan('directory.create.error.01'));
        } else if (!gThis.validDirectoryname(lName)) {
            jQuery('#directory\\.create\\.label').css('color', 'red').text(lan('directory.create.error.02'));
        } else {
            var lParams = {
                'act': 'hom-fil.createdirectory',
                'path': lPath,
                'name': lName
            };
            Flow.Std.ajxUpd(lParams);
            ...
            var lParams = {
                'act': 'hom-fil.getdirectories',
                'div': 'directories',
                'relativedirectory': lPath,
                'directoryname': lName,
                'loading_screen': true
            };
            Flow.Std.ajxUpd(lParams);

            jQuery('#dialog\\.directory\\.create').modal('hide');
        }
    };

    this.renameDirectory = function() {
        var lPath = gRelativeDirectory + gDirectoryName;
        var lName = jQuery('#directory\\.rename').val();
        ...
        if (gThis.isEmpty(lName)) {
            ...
            jQuery('#dialog\\.directory\\.rename').modal('hide');
        }
    };
    ...
    this.removeDirectory = function() {
        ...
        jQuery('#dialog\\.directory\\.remove').modal('hide');
    };

    this.uploadFileNonFlink = function() {
        jQuery('input[name="relativedirectory"]').val(gRelativeDirectory);
        jQuery('input[name="directoryname"]').val(gDirectoryName);

        jQuery('#form\\.nonflink\\.file\\.upload').submit();
        jQuery('#dialog\\.nonflink\\.file\\.upload').modal('hide');
    };

    this.uploadFileFlink = function() {
    };

    this.renameFile = function(aPath) {
    };

    this.removeFile = function(aFilename) {
        jQuery('#dialog\\.file\\.remove').modal('show');

        jQuery('#dialog\\.file\\.remove').on('click', '#okay', function (e) {
            jQuery.ajax({
                type : 'post',
                url : 'index.php?act=hom-fil.removefile',
                data : {
                    path : gRelativeDirectory + gDirectoryName,
                    name : aFilename
                },
                success : function(aData) {
                }
            ...
            jQuery('#dialog\\.file\\.remove').modal('hide');
        ...
        jQuery('#dialog\\.file\\.remove').on('click', '#cancel', function (e) {
            jQuery('#dialog\\.file\\.remove').modal('hide');
        });
    };
};
Flow.Files = new Flow.Files();

Flow.Mails = {
  setActive : function(aEmlid, aStatus) {
		var ajx = jQuery.ajax({
	      url:'index.php?act=ajx.setActive',
	      method:'POST',
        dataType: 'json',
	      data : { status : aStatus, emailid : aEmlid }
		});
    ajx.success(function(lRes) {
      jQuery('.'+aEmlid).fadeOut();
    });
  },

  setMail : function(aEmlid, aStatus) {
    var ajx = jQuery.ajax({
	      url:'index.php?act=ajx.setMail',
	      method: 'POST',
        dataType: 'json',
	      data : { status : aStatus, emailid : aEmlid }
		});
    ajx.success(function(lRes) {
      if(aStatus === 0) {
        jQuery('.r'+aEmlid).html("<img alt='open' src='img/wave8/ico/16/emailopen.gif'>");
        jQuery('.r'+aEmlid).attr('onclick','Flow.Mails.setMail("'+aEmlid+'",1)');
        jQuery('.'+aEmlid).children().find('a').removeClass('b');
      }
      else {
        jQuery('.r'+aEmlid).html("<img alt='closed' src='img/wave8/ico/16/email.gif'>");
        jQuery('.r'+aEmlid).attr('onclick','Flow.Mails.setMail("'+aEmlid+'",0)');
        jQuery('.'+aEmlid).children().find('a').addClass('b');
      }
    });
  }
};
