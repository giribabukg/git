  ...
  */
  ...
  protected function actSelview() {
    ...
    if ($lRow = $lQry -> getDat()) {
      ...
      $lUsr -> setPref($this -> mMod.'.pvs', $lRow['pvs']);
    }
    $this -> redirect();
  }
  ...
  protected function actAllview() {
    ...
    if (!empty($lPrf)) {
      ...
      foreach ($lPrf as $lKey => $lVal) {
        ...
        $lSql = 'INSERT INTO al_sys_pref';
        $lSql.= ' SET mand='.MID.',code='.esc($lModKey).',val='.esc($lVal);
  ...
  protected function actCpy() {
    ...
    if (CCor_Cfg::get('wec.available') && !CCor_Cfg::get('dalim.available')) {
  ...
  protected function actFlag() {
    ...
    if (isset($lAllFlags[$lFlag]) AND isset($lShowFlags[$lFlag])) {
      ...
      if (($lShow !== '-' AND $lShow) OR //[TYP => $lShow muss im Vgl. zuerst stehen] Eingeladen und an der Reihe ODER
        ...
        $lDlg = new CJob_Flag_Dialog($this -> mSrc, $lJobId, $lId, $lCap, $lVote, $lFlag, $lApl, $lImg);
        ...
        $this -> dbg(MID.','.$this -> mSrcCnt.','.$lId.': User('.$lUid.') isn\'t in the series for Flag '.$lFlag.' or has still confirmed.', mlInfo);
        $this -> redirect($this -> getStdUrl().'.edt&jobid='.$lId);
      } else {
        $this -> dbg(MID.','.$this -> mSrcCnt.','.$lId.': User('.$lUid.') has no Rights for Flag '.$lFlag.'!', mlWarn);
        $this -> redirect($this -> getStdUrl().'.edt&jobid='.$lId);
      }
    } else {
      $this -> dbg(MID.','.$this -> mSrcCnt.','.$lId.': User('.$lUid.'): Flag '.$lFlag.' doesn\'t exist i._fla OR i._states!', mlError);
      $this -> redirect($this -> getStdUrl().'.edt&jobid='.$lId);
    }
  }
      ...
      if ($lSetFlagState) {
        ...
        if ($lConfirm) {
          ...
          if ($lCopyJobTo !== '') {
            ...
            $this -> redirect('index.php?act=job-'.$lCopyJobTo.'.cpy&jobid='.$lId.'&src='.$this -> mSrcCnt.'&target='.$lCopyJobTo);
          } elseif ($lCopyTaskTo !== '') {
            ...
            $lSql.= ' WHERE '. $lJobIdColumn.' ='.esc($lId);
            $lAssignedProId = CCor_Qry::getInt($lSql);
            if ($lAssignedProId != FALSE) {
              $this -> dbg('Assigned ProjectId of reference job:'.$lAssignedProId);
              $this -> redirect('index.php?act=job-'.$lCopyTaskTo.'.cpy&jobid='.$lId.'&src='.$this -> mSrcCnt.'&target='.$lCopyTaskTo.'&proid='.$lAssignedProId);
            } else {
              $this -> redirect('index.php?act=job-'.$lCopyTaskTo.'.cpy&jobid='.$lId.'&src='.$this -> mSrcCnt.'&target='.$lCopyTaskTo);
            }
          } else {
            $lBeat = new CJob_Workflow_Heartbeat($this -> mSrcCnt, $lId);
            $lBeat -> heartBeat();
  ...
  protected function actCnf() {
    ...
    if ($lNoStop4Crp) {
      ...
      if ($lHasStepped) {
        ...
        $lStepInfos = $lStepClass -> GetStepInfos($lSid);
        $lToStatus = intval($lStepInfos['status']);
        $lUpdate = array(
          'webstatus' => $lToStatus,
        ...
        if ($lCopyJobTo !== '') {
          // Copy Job without his Project Assigment.
          CJob_Utl_Shadow::reflectUpdate(strtolower($this -> mSrcCnt), $lJobId, $lUpdate); // this does not do an automatic update, but only updates the webstatus to ensure the my task list is up-to-date

          // CJob_Utl_Shadow::reflectData(array('jobid' => $lJobId, 'webstatus' => $lToStatus)); // this is an alternative solution to the reflectUpdate in the previous line 
          // CJob_Utl_Shadow::reflectReportData(array('jobid' => $lJobId, 'webstatus' => $lToStatus)); // this is an alternative solution to the reflectUpdate in the previous line
          ...
          // Copy Job with his Project Assigment.
          // Find assigned ProjektId.
          if (CCor_Cfg::get('projects.variant', FALSE) == 'new') {
            $lSql = 'SELECT id,pro_id FROM al_job_sub_'.MID.' WHERE jobid_item='.esc($lJobId);
          } else {
            $lJobIdColumn = 'jobid_'.$this -> mSrcCnt;
          ...
          if ($lAssignedProItemId !== FALSE) {
            $lRedirectUrl = 'index.php?act=job-'.$lCopyTaskTo.'.cpy&jobid='.$lJobId.'&src='.$this -> mSrcCnt.'&target='.$lCopyTaskTo.'&proid='.$lAssignedProId.'&itmid='.$lAssignedProItemId.'&copytask=1';
            if (CCor_Cfg::get('projects.variant', FALSE) == 'new') $lRedirectUrl = 'index.php?act=job-'.$lCopyTaskTo.'.newmastersub&pid='.$lAssignedProId.'&src='.$this -> mSrcCnt.'&target='.$lCopyTaskTo.'&mid='.$lAssignedProItemId;
            $this -> dbg('Assigned ProjectId of reference job:'.$lAssignedProId);
            $this -> redirect($lRedirectUrl);
          } else {
  ...
  */
  ...
  protected function actSassignprj() {
    ...
    if ($lFromProId) {
      ...
      $lSQL.= ' WHERE `mand`='.MID;
      $lSQL.= ' AND pro_id ='.esc($lFromProId);
      $lSQL.= ' AND jobid='.esc($lJobId).';';
      $lQry = new CCor_Qry();
  ...
  public static function isFlagConfirmed($aJob) {//used in job/form & job/bar
    $lCrpFlags = array();
    $lAllFlags = CCor_Res::get('fla');
    if (!empty($lAllFlags)) {
   ...
   * 
   * @param string $aType
   * @param string $aCanDeselectCountry
   */
  protected function selectAplDialog($aType = 'apl', $aCanDeselectCountry = FALSE) {
    $lJobId = $this -> getReq('jobid');
    ...
    $lObj = new CJob_Apl_Preview($lJobId);
    $lObj->killSession();
    ...
    if ($lLoop->hasCompletedLoop($aType) OR $aCanDeselectCountry ) {
  ...
  protected function actSedt() {
    $lClientID = $this -> getReq('clientid');
    self::checkMand($lClientID);

    $lJobID = $this -> getReq('jobid');
    $lStepID = $this -> getReq('step');
    $lPage = $this -> getReq('page', 'job');
    $lWebStatus = $this -> getReq('webstatus');
    ...
    if ($lMod -> update()) {
      ...
      if ($lStepID > 0) {
        $this -> redirect('index.php?act=job-'.$this -> mSrcCnt.'.step&sid='.$lStepID.'&jobid='.$lJobID.'&webstatus='.$lWebStatus);
      } else {
        $lBeat = new CJob_Workflow_Heartbeat($this -> mSrcCnt, $lJobID);
        $lBeat -> heartBeat();
      }
    ...
    $this -> redirect('index.php?act=job-'.$this -> mSrcCnt.'.edt&jobid='.$lJobID.'&page='.$lPage);
  }
  ...
  protected function updateProjectItem($aJobId, $aUpdates) {
    ...
    $lSubID = CCor_Qry::getInt($lSQL);

    if (empty($lSubID)) return;

    $lUpdates = $aUpdates;
    $lMod = new CJob_Pro_Sub_Mod();
    $lMod -> forceVal('id', $lSubID);
    foreach ($lUpdates as $lKey => $lValue) {
      if (is_array($lExcludeFields) && in_array($lKey, $lExcludeFields)) {
        continue;
      }

      if (('*' == $lUpdateFields) ||(is_array($lUpdateFields) && in_array($lKey, $lUpdateFields))) {
  ...
  protected function actNewsub() {
    ...
    if (!empty($lPid)) {
      ...
      $lFrm->addBtn('act', lan('job-back-to-pro'), 'go("index.php?act=job-pro-sub&jobid=' . $lPid . '")', 'img/ico/16/cancel.gif', 'button', array('class' => 'btn w200'));
    }
  ...
  protected function actCreateQL() {
    $this->mJobId = $this->getVal('jobid');
    $this->mSrc = $this->getVal('src');

    $lMod = new CQuestions_Mod();
    $lMod->createQL($this->mJobId, $this->mSrc);

    $this->redirect('index.php?act=job-'.$this->mSrc.'-questions&jobid='.$this->mJobId);
  }

  protected function actRefImport() {
    $this->mJobId = $this->getVal('jobid');
    $this->mRefJobId = $this->getVal('refjobid');
    $this->mSrc = $this->getVal('src');

    //Import QL Data
    $lQLMod = new CQuestions_Mod();
    $lQLMod->importQL($this->mJobId, $this->mRefJobId, $this->mSrc);
    //Import Phrase Data
    //To Be continued...
  ...
  protected function getQuestions() {
    ...
    $lQry = "SELECT * FROM al_questions_master ORDER BY 'domain'";
    ...
    foreach($lQry as $lList) {
      ...
      $lQuest = 'SELECT * FROM al_questions_items WHERE master_id = '.$lList['id'].' ORDER BY name_'.LAN;
