 ...
 * @version $Rev: 13860 $
 * @date $Date: 2016-05-12 16:33:21 +0200 (Thu, 12 May 2016) $
...
#      $lUpdateReport -> setTimingInReportTable($aDisplay, $aTime, $this -> mJobId);
  ...
  public function doStep($aStepId, $aMsg = '', $aAdd = '', $aIgn = array(), $lMsg_Flag = array(), $aStartApl = FALSE) {
    ...
    if ('pro' != $this -> mSrc) {
      $lProItemSrc = (CCor_Cfg::get('view.projekt.joblist.tree', false)) ? 'jobid_item' : 'jobid_'.$this -> mSrc;
      $lSql = 'UPDATE al_job_sub_'.MID.' SET webstatus="'.$lSta.'" WHERE '.$lProItemSrc.'="'.$this -> mJobId.'"';
    } else {
    ...
    $this -> updateJobFlags($lRow, $lDraftJobId, $lMsg_Flag, $lStp, $lFrom, $aIgn, $lHisInsertId);
  ...
  protected function updateJobFlags($aRow, $aDraftJobId, $aMsg_Flag, $aStp, $aFrom, $aIgn, $aHisInsertId) {
    $lRow = $aRow;
    $lDraftJobId = $aDraftJobId;
    $lMsg_Flag = $aMsg_Flag;
    $lStp = $aStp;
    $lFrom = $aFrom;
    $lHisInsertId = $aHisInsertId;
  ...
  public function doStepIndependent($aStepId, $aMsg = '', $aAdd = '', $aIgn = array()) {
    ...
    $lSql = 'SELECT * FROM al_crp_step WHERE id='.$lStp;
    ...
    $this -> updateJobFlags($lRow, $this -> mJobId, $aMsg, $lStp, '', $lIgn, $lHisInsertId);
    return TRUE;
  }
  ...
  public function ConfirmFlag($aConfFlag = '', $lCap = '', $aMsg = '', $aWebstatus = '') {
    ...
    if (!empty($aConfFlag)) {
      ...
      foreach ($lFlag as $lF) {
        ...
        $IsConf = CApp_Apl_Loop::isFlagConfirmed($this -> mSrc, $lDat['jobid'], $lFlagId);
  ...
  protected function onCopy($aOldJobId, $aNewJobId) {
    ...
    $lSql = 'UPDATE al_job_sub_'.intval(MID).' SET jobid_'.$this -> mSrc.'='.esc($lNew).' WHERE jobid_'.$this -> mSrc.'='.esc($lOld).';';
    $lQry -> query($lSql);

    $lSql = 'UPDATE al_usr_recent SET jobid='.esc($lNew).' WHERE jobid='.esc($lOld).';';
    $lQry -> query($lSql);

    $lSql = 'UPDATE al_usr_bookmark SET jobid='.esc($lNew).' WHERE jobid='.esc($lOld).';';
    $lQry -> query($lSql);

    $lSql = 'UPDATE al_job_his SET src_id='.esc($lNew).' WHERE src_id='.esc($lOld).' AND mand='.intval(MID).';';
    $lQry -> query($lSql);

    $lSql = 'UPDATE al_job_files SET jobid='.esc($lNew).' WHERE jobid='.esc($lOld).';';
    $lQry -> query($lSql);
    ...
    $lSql = 'UPDATE al_job_shadow_'.intval(MID).' SET jobid='.esc($lNew).' WHERE jobid='.esc($lOld).';';
    $lQry -> query($lSql);

    $lSql = 'UPDATE al_job_apl_loop SET jobid='.esc($lNew).' WHERE jobid='.esc($lOld).' AND typ!=0 AND mand='.intval(MID).';';
    $lQry -> query($lSql);

    $lSql = 'UPDATE al_job_sku_sub_'.intval(MID).' SET job_id='.esc($lNew).' WHERE job_id='.esc($lOld).';';
    $lQry -> query($lSql);

    $lSql = 'UPDATE al_cms_ref_job SET jobid='.esc($lNew).' WHERE jobid='.esc($lOld).';';
    $lQry -> query($lSql);
    
    if (CCor_Cfg::get('extended.reporting')) {
    	$lSql = 'UPDATE al_job_shadow_'.intval(MID).'_report SET jobid='.esc($lNew).' WHERE jobid='.esc($lOld).';';
    	$lQry -> query($lSql);
    }

    if(CCor_Cfg::get('xchange.available')){
    	$lSql = 'UPDATE al_xchange_jobs_'.intval(MID).' SET x_jobid='.esc($lNew).' WHERE x_jobid='.esc($lOld).';';
    	$lQry -> query($lSql);
    }
