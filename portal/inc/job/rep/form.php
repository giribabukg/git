 ...
 */
class CInc_Job_Rep_Form extends CJob_Form {
...
#$lCnd = $lUsr -> getJobCond();
#echo '<pre>---form.php---'.get_class().'---';var_dump($lUsr,$lCnd,'#############');echo '</pre>';
    ...
    $this -> addBtn('act', lan('lib.cancel'), 'go("index.php?act=job-rep")', 'img/ico/16/cancel.gif', 'button', array('class' => 'btn w200' ));
    if (!empty($this -> mJobId)) {
      $this -> addBtn('act', lan('lib.print'), 'pop("index.php?act=job-rep.prn&jobid='.$this -> mJobId.'")', 'img/ico/16/print.gif', 'button', array('class' => 'btn w200' ));
    }
    $this->addPrjButtons();
    ...
    $this -> mTabs = new CJob_Rep_Tabs($this -> mJobId, $aPage);
    ...
    if (($lUsr -> canEdit('job-wec-id')) && ($this->mJobId) &&(substr($this->mJobId, 0, 1) != 'A')) {
      ...
      $lAtt = array();
      $lAtt['class'] = 'btn w200';
      $this -> addBtn('act', lan('wec.menu'), 'go("index.php?act=job-rep.wec&src='.$this -> mSrc.'&direct=1&jobid='.$this -> mJobId.'")', 'img/ico/16/wec.gif', 'button', $lAtt);
    }
  ...
  protected function addPrjButtons() {
    $lUsr = CCor_Usr::getInstance();
    if ($this -> canAssign()) {
      $this -> addBtn('act', lan('job.assignprj'), 'go("index.php?act=job-rep.assignprj&jobid='.$this -> mJobId.'")', 'img/ico/16/next-hi.gif', 'button', array('class' => 'btn w200' ));
    }
    ...
    if ($this -> canAssignSkuSub()) {
      $this -> addBtn('act', lan('job.assignskusub'), 'go("index.php?act=job-rep.assignskusub&jobid='.$this -> mJobId.'")', 'img/ico/16/next-hi.gif', 'button', array('class' => 'btn w200' ));
    }
  }
