...
class CInc_Job_Part extends CCor_Tpl {
  ...
  public function __construct($aSrc, $aKey, & $aFac, & $aJob, $aOld = TRUE) {
    ...
    $lDis = (empty($lPrf)) ? 'collapse' : 'expand';
    $this -> setPat('collapse', $lDis);
  ...
  #echo '<pre>---part.php---'.get_class().'---';var_dump($lDef['alias'],$lVal,$lFid ,$lSta,$lIsSteered,'#############');echo '</pre>';
      ...
      $i = 1;
      foreach ($lFie as $lFid => $lDef) {
        if (!empty($lDef['native_core'])) {
          $this->setPat('val.'.$lDef['alias'], $i.'.<div class="box w200 p4" style="min-width:250px; width:250px; background-color:#fc6; margin:1px;">'.$lDef['native_core'].'</div>');
          $i++;
        }
      }
      */
