...
protected function getSrcPanel() {
 ...
 protected function getCountryApl($aCtr = '', $aType = 'apl') {
   ...
   $lRet.= '<select id="'.$lEveId.'" name="eve['.$this->encKey($aCtr).']" size="10" class="w300 apl-tpl" onchange="Flow.apl.getOverview(this.value,\''.$lJid.'\',\''.$aCtr.'\',\''.$lNum.'\',\''.$this->mCanDeselectCountry.'\')">';
 ...
 protected function getAddRevisorButtons($aJid, $aCtr, $aNum, $aEveId, $aCanDeselect = FALSE) {
   $lUsr = CCor_Usr::getInstance();
   $lCanRead = $lUsr -> canRead('job.step.addrevisor');
