...
class CInc_Job_Pro_Mod extends CJob_Mod {
  ...
  protected function doUpdate() {
    ...
    if ($lRet) {
    	$lSql = 'SELECT jobid FROM al_job_pro_'.intval(MID).' WHERE id="'.$this -> mJobId.'"';
    	$lJobid = CCor_Qry::getStr($lSql);
      CJob_Utl_Shadow::reflectUpdate($this -> mSrc, $lJobid, $this -> mUpd);
    }
    return $lRet;
  }
  ...
  protected function doInsert() {
    ...
    $this -> mVal['src'] = 'pro';
    $this -> mVal['jobid'] = CInc_Job_Writer_Portal::getNewPdbId('pro');
    ...
    if ($this -> mTest) {
      ...
      if ($lRet) {
        ...
        CJob_Utl_Shadow::reflectInsert($this -> mSrc, $this -> mVal['jobid'], $this -> mVal, array());
      } else {
        $this -> mInsertId = NULL;
      }
      return $lRet;
