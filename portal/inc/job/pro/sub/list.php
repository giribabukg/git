 ...
 * @version $Rev: 13435 $
 * @date $Date: 2016-04-13 15:14:45 +0200 (Wed, 13 Apr 2016) $
...
class CInc_Job_Pro_Sub_List extends CHtm_List {
  ...
  protected function createToolBar() {
    ...
    $lRefresh = $lUsr -> canRead('job-pro-status.actualize'); // TODO: what is meant is >refresh< or >update< and not >actualise< or >actualize< - linguists would kill us!
    if (1 == $lRefresh) {
      $this -> addBtn(lan('job-pro-status.actualize'), 'go(\'index.php?act='.$this -> mMod.'.stataktual&projectid='.$this -> mJobId.'\')', 'img/ico/16/mt-4.gif'); // TODO: why does mJobID hold the mProjectID?
    }
  ...
  protected function preLoad() {
    ...
    $lSQL = 'SELECT sub_id,pro_status';
    $lSQL.= ' FROM al_job_pro_crp';
    $lSQL.= ' WHERE mand='.MID;
    $lSQL.= ' AND jobid IN ('.$lAllJobids.');';
    $lQry = new CCor_Qry($lSQL);
    $lSubCrp = array();
    foreach ($lQry as $lRow) {
  ...
  protected function getTdIs_master() {
    ...
    if ('X' == $lMas) {
      ...
      $lRet.= '<a class="nav" href="javascript:Flow.Std.cnf(\''.$lDelLink.'\', \'cnfMAssOut\')">';
      ...
      if (!empty($lMaster_Id)){
        ...
        $lRet.= '<a class="nav" href="javascript:Flow.Std.cnf(\''.$lDelLink.'\', \'cnfVAssOut\')">';
