<?php
class CInc_Job_Mis_Apl_Cnt extends CCor_Cnt {
  ...
  protected function actStd() {
    ...
    $lVie = new CJob_Mis_Tabs($lJid, 'apl');
    $lRet.= $lVie -> getContent();

    $lFrm = new CJob_Apl_List('mis', $lJid);
    $lRet.= $lFrm -> getContent();

    #$lApl = new CApp_Apl_Loop('mis', $lJid, 'apl');
    #$lApl -> createLoop('2008-07-01');

    $lFrm = new CJob_Mis_Apl_Form('job-mis-apl.sedt', $lJid, $lJob, $lPag);
    $lRet.= $lFrm -> getContent();
  ...
  protected function actNewmail() {
    ...
    $lVie = new CJob_Apl_Mailform('mis', $lJid, $lJob);
    $lRet.= $lVie -> getContent();
  ...
  protected function actSnewmail() {
    ...
    $lMod = new CJob_His_Mod('mis', $lJid, htMail);
   ...
   # $lMod -> insert(); //braucht nicht dokumentiert zu werden.
    ...
    $this -> redirect('index.php?act=job-mis-apl&jobid='.$lJid);
  }
