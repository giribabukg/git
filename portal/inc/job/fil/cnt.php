...
class CInc_Job_Fil_Cnt extends CCor_Cnt {
  ...
  public function checkFileSize() {
    $lCat = $this -> getVal('category');
    $lFileSize = $this -> getInt('size');
    $lExt = $this -> getVal('ext');
    
    $lPermittedSize = CCor_Cfg::getFallback('max.file.size.'.$lCat, 'max.file.size', 52428800);
    return ($lFileSize <= $lPermittedSize) ? TRUE : FALSE;
  }
...
// $lQry -> setClient($lStub);
  ...
  protected function actSuploadpixelboxx() {
    ...
    $lUpload->uploadToPixelboxx($lTemp, $lOldName, array('category' => $lCat));
  ...
  protected function actSuploadglobalvision() {
    $lSrc = $this -> getReq('src');
    $lJid = $this -> getReq('jid');
    $lNam = $this -> getReq('name');
    $lParam = array('dest' => 'globalvision');

    $lFinder = new CApp_Finder($lSrc, $lJid);
    $lDir = $lFinder -> getDynPath(CCor_Cfg::get('flink.destination.pdf.dir'));
    $this -> msg('CInc_Job_Fil_Cnt uses actSuploadglobalvision at: '.$lDir);
      
    $lUpl = new CJob_Fil_Upload($lSrc, $lJid);
    $lUpl -> uploadFile($lDir.$lNam, $lNam, $lParam);
  }
  ...
  protected function actFilelinkSupload() {
    ...
    $lAlias = CCor_Res::extract('alias', 'param', 'fie', array('typ' => 'file'));
    $lParam = (isset($lAlias[$lFId])) ? toArr($lAlias[$lFId]) : NULL;
    $lParam['dest'] = (isset($lParam['dest'])) ? $lParam['dest'] : $lSub;
  ...
  protected function actFilelinkUploadReg() {
      ...
      $lClass = 'CJob_'.$lSrc.'_Mod';
      $lObject = new $lClass($lJId);
  ...
  protected function actNonFlinkFileUpload() {
    $lRelativeDirectory = $this -> getReq('relativedirectory');
    $lDirectoryName = $this -> getReq('directoryname');

    $lAppFinder = new CApp_Finder('app');
    $lAppFinder -> setMid(MID);
    $lDir = $lAppFinder -> getPath();
    ...
    if (!$lRes) {
      ...
      CCor_Usr::insertJobFile(NULL, NULL, 'app', $lRes, NULL, NULL, $lDir.$lRelativeDirectory.$lDirectoryName);
    }

    $lParams = array(
      'act' => 'hom-fil.getfiles',
      'div' => 'files',
      'relativedirectory' => $lRelativeDirectory,
      'directoryname' => $lDirectoryName,
      'loading_screen' => TRUE
    );
    $lParamsJSONEnc = json_encode($lParams);
    echo '<script type="text/javascript">parent.window.Flow.Std.ajxUpd('.$lParamsJSONEnc.')</script>';
  ...
  protected function actGlobalvision() {
    $lSrc = $this -> getReq('src');
    $lJid = $this -> getReq('jid');
    $lNam = $this -> getReq('fname');
    $lOnlyGv = $this -> getReq('OnlyGv');
  
    $lFin = new CApp_Finder($lSrc, $lJid);
    $lDir = $lFin -> getDynPath(CCor_Cfg::get('flink.destination.pdf.dir'));
    $lFil = $lDir . $lNam;
    $this -> msg('lFil copy ' . $lFil, mtUser);
    if (is_readable($lFil)) {
      $lDir = CCor_Cfg::get('gv.hotfolder.pdf');
      $lNewFile = $lDir . DS . $lNam;
      if ( ! copy($lFil, $lNewFile)) {
        $this -> msg('Failed to copy ' . $lFil, mtUser);
      } else {
        // TODO: upload to webcenter/dalim
        # Uploaded to GV and updated to al_job_files by 'Y'
        CCor_Usr::uploadedJobFileToGv($lSrc, $lJid, 'pdf', $lNam);
        // dalim or webcenter
        $lDalim = CCor_Cfg::get('dalim.available', FALSE);
        $lWebCenter = CCor_Cfg::get('webcenter.available', FALSE);
  
        if ($lOnlyGv !='OnlyGv'){
          if ($lDalim) {
            $lUtil = new CApi_Dalim_Utils();
            $this -> msg('Register DOC ' . $lJid . '/' . $lNam, mtApi, mlInfo);
            $lUtil -> registerDocument($lJid . '/' . $lNam);
          }
          if ($lWebCenter) {
            //CCor_Usr::uploadedJobFile($lSrc, $lJid, 'pdf', $lNam);
            $lSql = 'SELECT wec_prj_id FROM al_job_shadow_' . MID . ' ';
            ...
            if ($lOnServer) {
              ...
              $lQry = new CApi_Wec_Query_Upload($lWec);
              $lRes = $lQry -> upload($lWecPrjId, $lURL . $lNam, $lNam);
              // <error> <code>023</code> <message>Document filespec is not accessible by the application server</message> </error>
              //
              if ($lRes) {
                // if upload is success update the db
                CCor_Usr::insertJobFile($lSrc, $lJid, 'pdf', $lNam, 1105);
                CCor_Usr::uploadedJobFile($lSrc, $lJid, 'pdf', $lNam);
        ...
        $this -> msg('Copied success location from ' . $lFil . ' to ' . $lNewFile, mtUser);
        $lArr = array();
        $lArr['jid'] = $lJid;
        $lArr['src'] = $lSrc;
        $lArr['name'] = $lNam;
    ...
    $this -> redirect('index.php?act=job-' . $lSrc . '-fil&jobid=' . $lJid . '&sub=pdf');
  }
  
/*   public function actCheckfilesize(){
    $lSize = $this -> getReq('size');
    $lCategory = $this -> getReq('category');
    $lExt = $this -> getReq('ext');
    echo json_encode(CJob_Fil_Upload::checkFileSize($lCategory, $lSize, $lExt));
  } */
}