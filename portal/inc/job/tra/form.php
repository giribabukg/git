 ...
 */
 class CInc_Job_Tra_Form extends CJob_Form {
  ...
  public function __construct($aAct, $aJobId = 0, $aJob = NULL, $aPage = 'job') {
    ...
    $this -> mTabs = new CJob_Tra_Tabs($this -> mJobId, $aPage);
    ...
    if (($lUsr -> canEdit('job-wec-id')) && ($this -> mJobId) && (substr($this -> mJobId, 0, 1) <> 'A')) {
      $lAtt = array();
      $lAtt['class'] = 'btn w200';
      $this -> addBtn('act', lan('wec.menu'), 'go("index.php?act=job-tra.wec&src=tra&direct=1&jobid='.$this -> mJobId.'")', 'img/ico/16/wec.gif', 'button', $lAtt);
    }
