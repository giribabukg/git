<?php
class CInc_App_Event_Action_Email_Role extends CApp_Event_Action {

  public function execute() {
    $lSender = new CApp_Sender('rol', $this -> mParams, $this -> mContext['job'], $this -> mContext['msg']);
    return $lSender -> execute();
  }

  public static function getParamDefs($aType) {
    $lArr = array();
    $lRol = CCor_Res::extract('alias', 'name', 'rol');
    $lFie = fie('sid', 'Role', 'select', $lRol);
    $lArr[] = $lFie;
  ...
  public static function paramToString($aParams) {
    ...
    if (isset($aParams['sid'])) {
      $lSid = $aParams['sid'];
      $lArr = CCor_Res::extract('alias', 'name', 'rol');
      $lRet.= (isset($lArr[$lSid])) ? $lArr[$lSid] : '[unknown role '.$lSid.']';
    } else {
      $lRet.= '[empty group]';
    }
    $lRet.= ', ';
