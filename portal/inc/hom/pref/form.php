...
class CInc_Hom_Pref_Form extends CCor_Ren {
  ...
  protected function setSecurityTokenTag() {
    $lRet = '<input type="hidden" name="'.sec_token.'" value="'.$_SESSION[sec_token].'" />'.LF;
    return $lRet;
  }
  ...
  protected function getHomePrefStart($aUsr) {
    ...
    if ($lUsr -> canRead('pixelboxx') && CCor_Cfg::get('pbox.available')) 
    		  $lArr['pixelboxx'] = lan('pixelboxx.menu');
  ...
  protected function getDalimPrefForm() {
    ...
    $lRet.= '<td>';
    $lCur = $lUsr -> getPref('dalim.viewer', 'html5');
