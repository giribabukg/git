 ...
 * @version $Rev: 13827 $
 * @date $Date: 2016-05-11 16:13:45 +0200 (Wed, 11 May 2016) $
...
class CInc_Hom_Wel_MyTasks extends CCor_Ren {
  ...
  protected $mWithoutLimit = FALSE; // default as to be found in Alink and table iterator
  ...
  protected function getJobs($aMod = '') {
    ...
    foreach ($this -> mJobList as $lKey => $lValue) {
      foreach ($this -> mFlagTyp[$lKey] as $lInnerKey => $lInnerValue) {
        if ('apl' == $lInnerKey) {
          $lAplResJobIDs[] = $lKey;
        } elseif ('role' == $lInnerKey) {
          $lRoleResJobIDs[] = $lKey;
        } else {
          $lFlagResJobIDs[] = $lKey;
        }
  ...
  protected function getProjectJobs($aMod = '') {
    $lAplRes = array();
    $lFlagRes = array();
    $lRoleRes = array();
    ...
    $this -> mIte = new CCor_TblIte('al_job_pro_'.MID, $this -> mWithoutLimit);
    ...
    if (CCor_Cfg::get('job.writer.default') == 'portal') {
      $this -> mIteWithJobId = $this -> mIte -> getArray();
    } else $this -> mIteWithJobId = $this -> mIte -> getArray('id');
    ...
    $this -> mAplIte  = array_merge($this -> mAplIte, $lAplRes);
    $this -> mFlagIte = array_merge($this -> mFlagIte, $lFlagRes);
    $this -> mRoleIte = array_merge($this -> mRoleIte, $lRoleRes);
  }
  ...
  protected function getAplTaskContent() {
    ...
    $this -> getJobs('apl');
    if (CCor_Cfg::get('hom-wel-apl.showprojectjobs', FALSE) == TRUE) {
      $this -> getProjectJobs('apl');
    }
  ...
  protected function getFlagTaskContent() {
    ...
    $this -> getJobs('flag');
    if (CCor_Cfg::get('hom-wel-flag.showprojectjobs', FALSE) == TRUE) {
      $this -> getProjectJobs('flag');
    }
  ...
  protected function getRoleTaskContent() {
    ...
    $this -> getJobs('role');
    if (CCor_Cfg::get('hom-wel-role.showprojectjobs', FALSE) == TRUE) {
      $this -> getProjectJobs('role');
    }
