...
//       $lRes = $this -> removeStatusReRs($lRes); // Remove Jobs which has STATUS = RE or RS or G
  ...
  protected function getTdApl() {
    ...
    if ($lLoopId != 0) {
      ...
      if ($lIsAplCompleted) {
        $lRet = $lApl -> getAplCommitList($lLoopId, $this -> mCurLnk);
  ...
  protected function getLink() {
    $lSrc = $this -> getVal('src');
    $lJobId = $this -> getVal('jobid');
    $lId = ($lSrc == 'pro') ? $this -> getVal('id') : $lJobId;

    switch ($this -> mMyUrl) {
      case 'apl':
        $lApl = new CInc_App_Apl_Loop($lSrc, $lJobId);
        $lAplType = $lApl -> getLoopById($this -> mRow['loop_id']);
        if ($lAplType['apl_mode'] == 2) $lRet = $this -> mStdLnk.$lSrc.'.edt&jobid='.$lId;
        else $lRet = $this -> mStdLnk.'apl&src='.$lSrc.'&jobid='.$lId;
        break;
      case 'det':
        $lRet = $this -> mStdLnk.$lSrc.'.edt&jobid='.$lId.'&page=det';
        break;
      case 'fil':
        $lRet = $this -> mStdLnk.$lSrc.'-'.$this -> mMyUrl.'&jobid='.$lId;
        break;
      case 'flag':
        $lRet = $this -> mStdLnk.$this -> mMyUrl.'&src='.$lSrc.'&jobid='.$lId;
        break;
      case 'his':
      default:
        $lRet = $this -> mStdLnk.$lSrc.'.edt&jobid='.$lId;
    }
  ...
  protected function onBeforeContent() {
    ...
    foreach ($this -> mIte as $lRow) {
      if ($lRow['src'] == 'pro') {
      	$lSql = 'SELECT jobid FROM al_job_pro_'.intval(MID).' WHERE id='.esc($lRow['jobid']);
      	$lRow['jobid'] = CCor_Qry::getStr($lSql);
      }
