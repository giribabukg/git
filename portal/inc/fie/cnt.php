...
class CInc_Fie_Cnt extends CCor_Cnt {
  ...
  protected function actStructChange() {
    ...
    if (CCor_Cfg::get('job-all.use-view')) {
      // recreating the view should be done even if we have no SQL changes
      CJob_Writer_View::recreateView();
    }
    $this -> redirect();
  }
  ...
  protected function actStructProof() {
    ...
    if (!empty($lVal)) {
      ...
      if($lVal['src'] != "pro,sub,arc" AND $lVal['src'] != "arc"){
        $lAvail = '`avail` &'.constant('fs'.ucfirst($lVal['src']));
      }else{
        $lAvail = '1';
      }
      
      $lTable_al_fie = new CCor_Qry('SELECT id,src,alias,typ,flags,maxlen FROM al_fie WHERE `mand`='.MID.' AND '.$lAvail.' ORDER BY alias');
...
/*    `create_stamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  protected function actNativesearch() {
    $lDlg = new CFie_Form_Nativesearchdialog();
    $lDlg->render();
    exit;
  }
