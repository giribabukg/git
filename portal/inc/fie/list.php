...
class CInc_Fie_List extends CHtm_List {
  ...
  public function __construct() {
    ...
    $lValidate = CCor_Cfg::get('validate.available');
    $lCore = CCor_Cfg::get('core.available');
    ...
    if ($lCore) {
      $this -> addColumn('native_core',    'Core '.lan('fie.native'),      TRUE);
    }
    $this -> addColumn('typ',       lan('lib.type'),        TRUE);
    $this -> addColumn('param',     lan('lib.param'),       TRUE);
    $this -> addColumn('learn',     lan('fie-learn.list'),  TRUE);
    $this -> addColumn('maxlen',    lan('fie-maxlen'),      TRUE);
    if ($lValidate) {
      $this->addColumn('validate_rule', lan('validate.rule'), TRUE);
      $this->mValidateRules = CCor_Res::extract('id', 'name', 'validate');
    }
    if ($this -> mCanDelete) {
      $this -> addDel();
  ...
  protected function getTdValidate_rule() {
    $lTxt = '';
    $lRuleId = $this->getInt('validate_rule');
    if (isset($this->mValidateRules[$lRuleId])) {
      $lTxt = $this->mValidateRules[$lRuleId];
    }
