 ...
 * @version $Rev: 14703 $
 * @date $Date: 2016-06-27 14:10:25 +0200 (Mon, 27 Jun 2016) $
...
/**
 * Does the same as mysql_real_escape_string
 * but without the need of a DB Connection.
 * CAUTION: Please only use this method if
 * esc() is not working when there is no open
 * Database connection
 *
 * @param string $aStr String to convert
 * @return string Converted string
 */
function escWithoutDb($aStr) {
  if(!empty($aStr) && is_string($aStr)) {
 ...
 * checkEmail
 * Checks valid email
 * Check domain is either block or invalid
 * @param String  $aEmail
 * @return Boolean
 */

function isValidEmail ($aEmail) { 
  if ($isemail = filter_var($aEmail, FILTER_VALIDATE_EMAIL)) {
    list ($aEmail, $lDomain) = split('@', $aEmail);
    if (! getmxrr($lDomain, $mxhosts) ) {
      return FALSE;
    } else {
