...
class CInc_Htm_Mainmenu extends CCor_Ren {
  ...
  protected function addReportingMenu() {
    ...
    if ($this->mUsr->canRead('pixelboxx') &&  CCor_Cfg::get('pbox.available')) {
      $this->addItem('pixelboxx', $this->mLan->get('pixelboxx.menu'), 'mmElements', true);
    }
  }
  ...
  protected function addOptionMenu() {
    if ($this->mUsr->canRead('opt')) {
      ...
      if ($this->mUsr->canRead('questions')) {
        $lMen->addItem('index.php?act=questions', lan('questions-list.menu'));
      }

      $lPhrase = CCor_Res::extract('code', 'eve_phrase', 'crpmaster');
      if(array_filter($lPhrase) && $this->mUsr->canRead('cms-categories')) {
        $lMen->addItem('index.php?act=cms-categories', lan('cms-categories.menu'));
      }
      ...
      if ($this->mUsr->canRead('usr-external')) {
        $lMen->addItem('index.php?act=usr-external', lan('usr-external.menu'));
      }
      ...
      if (is_array($lCap)) {
        ...
        if ($lUrl == $this -> mKey) {
          if (strpos($this -> mKey, 'job') !== FALSE || strpos($this -> mKey, 'arc') !== FALSE) {
            ...
            if (!in_array($lSrc, $lAvailSrc) && isset($_GET['src'])) {
              $lSrc = $_GET['src'];
            }
