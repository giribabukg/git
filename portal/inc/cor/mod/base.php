 ...
 * @version $Rev: 14285 $
 * @date $Date: 2016-06-01 14:46:31 +0200 (Wed, 01 Jun 2016) $
...
abstract class CCor_Mod_Base extends CCor_Obj {
  ...
  public function getPost(ICor_Req $aReq, $aOld = TRUE) {
    ...
    $lTokenVal = $aReq ->getVal(sec_token);
    if(!isset($lTokenVal) || $lTokenVal !== $_SESSION[sec_token]) return;
  ...
  protected function getReqDate($aFie) {
    ...
    $lDateTime = new DateTime($lNew);
    $lNormalize = $lDateTime -> format(lan('lib.date.long'));

    $lDat = new CCor_Date();
    $lDat -> setInp($lNormalize);
