...
class CInc_Cor_TblIte extends CCor_Qry implements IteratorAggregate {
  ...
  public function __construct($aTbl, $aWithoutLimit = FALSE) {
    ...
    if ($aTbl == 'all') {
      ...
      $lSrc = ($lCountJobTypes == 0) ? array('rep') : $lJobTypes;
      $this -> mMultiTbls = $lSrc;
      $this -> mAddUserCondition = TRUE;
      $this -> mAddGroupCondition = TRUE;
    }
    $this->mUseView = CCor_Cfg::get('job-all.use-view', false);
  }
  ...
  public function getMultiCount($aField = NULL) {
    if ($this->mUseView) {
      return $this->getMultiCountFromView($aField);
    }
    return $this->getMultiCountFromSelect($aField);
  }

  public function getMultiCountFromView($aField = NULL) {
    $lField = empty($aField) ? '*' : '`'.$aField.'`';
    $lSql = 'SELECT COUNT('.$lField.') FROM al_job_all_'.MID.' ';
    $lSql.= $this -> getCondSql();
    return CCor_Qry::getInt($lSql);
  }

  public function getMultiCountFromSelect($aField = NULL) {
    if (empty($aField)) {
      $aField = '*';
    }
  ...
  public function getMultiIterator() {
    if ($this->mUseView) {
      return $this->getMultiIteratorFromView();
    }
    return $this->getMultiIteratorFromSelect();
  }

  public function getMultiIteratorFromView() {
    if (empty($this -> mFie)) {
      $lCols = '*';
    } else {
      $lCols = implode(',', $this -> mFie);
    }
    $lSql = 'SELECT '.$lCols.' FROM al_job_all_'.MID.' ';
    $lSql.= $this->getCondSql();
    $lSql.= ' AND webstatus >= 10 ';
    $lSql.= ' AND src IN (';
    foreach ($this -> mMultiTbls as $lSrc) {
      $lSql.= esc($lSrc).',';
    }
    $lSql = strip($lSql).') ';
  ...
  public function getMultiIteratorFromSelect() {
    if (empty($this -> mFie)) {
      $lColumns = '* ';
    } else {
      $lColumns = implode(',', $this -> mFie).' ';
    }
