...
#      elseif(0 == $lRow['mand'])
  ...
  protected function filterAdmin_level($aRet, $aVal) {
    $lRet = array();
    foreach ($aRet as $lId => $lRow) {
      if ($lRow['admin_level'] >= $aVal || $lRow['admin_level'] == '0') {
        $lRet[$lId] = $lRow;
      }
    }
    return $lRet;
    ...
    protected function filterMand($aRet, $aVal) {
    $lRet = array();
    foreach ($aRet as $lId => $lRow) {
      if ($lRow['mand'] >= $aVal || $lRow['mand'] == '0') {
        $lRet[$lId] = $lRow;
      }
    }
    return $lRet;
