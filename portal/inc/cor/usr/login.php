...
class CInc_Cor_Usr_Login {
  ...
  protected function setSecureTokenIntoSession() {
    $lApp = new CApp_Pwd();
    $lRandomToken = $lApp->createNewToken();
    $_SESSION['sec_token'] = $lRandomToken;
  }
  ...
  public function login($aUid, $aRow) {
    ...
    self::setSecureTokenIntoSession();
	...
	$lQry -> query('UPDATE al_usr SET lastlogin=NOW(), login_attempts=0 WHERE id="'.$lUid.'";');
