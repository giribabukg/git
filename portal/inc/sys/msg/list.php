...
class CInc_Sys_Msg_List extends CHtm_List {
  public function __construct($aMod = NULL) {
    ...
    $this->addColumn("mandName",    lan("lib.mand")         , true);
    ...
    if ($this -> mCanDelete) {
      $this -> addBtn(lan('lib.del.all'), 'javascript:Flow.Std.cnf("index.php?act=sys-msg.truncate", "cnfDel")', '<i class="ico-w16 ico-w16-del"></i>');
    }

    //Get List Content
    $this -> mIte = new CCor_TblIte('al_sys_msg m');
    $this -> mIte -> addField('m.id');
    $this -> mIte -> addField('m.mandName');
