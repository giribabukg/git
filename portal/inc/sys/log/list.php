...
class CInc_Sys_Log_List extends CHtm_List {
  ...
  public function __construct() {
    ...
    $this -> addColumn('datum', lan('lib.file.time.modification'), TRUE);
    $this -> addColumn('act',   'Act', TRUE);
    $this -> addColumn('user',  lan('lib.user'), TRUE);
    $this -> addColumn('msg',   lan('lib.msg'), TRUE, array('width' => '100%'));
    if ($this -> mCanDelete) {
      $this -> addBtn(lan('lib.del.all'), 'javascript:Flow.Std.cnf("index.php?act=sys-log.truncate", "cnfDel")', '<i class="ico-w16 ico-w16-del"></i>');
    }
