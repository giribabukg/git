...
class CInc_Svc_Queue extends CSvc_Base {
  ...
  protected function actWecthumb() {
    ...
    if (!$lDraft AND !$lArchived) {
      ...
      $lWectns -> downloadImages(array('jobid' => $lJobId, 'src' => $lSrc));
    }
  ...
  private function getRows($aRequest, $aResponse, $aLan, $aSrc) {
    ...
    //load flag values from shadow table
    $lArr = array_keys($lResponse);
    ...
    $lQry = new CCor_Qry($lSql);
    foreach ($lQry as $lRow) {
      $lResponse[$lRow['jobid']]['flags'] = $lRow['flags'];
    }
    ...
    foreach($lResponse as $this->mRow){
      ...
      foreach($lRequest->mCols as $this->mColKey => & $this->mCol){
        ...
        if($this->mColKey == 'flags'){ // flags
          ...
          $lRow .= '"' . utf8_decode( implode(", ", $lCell) ) . '";';
          ...
          $lCrpStageName = CCor_Res::extract('status', 'name_' . $lLan, 'crpstatus', $lCrpId);
          $lCrpStatusDisplay = CCor_Res::extract('status', 'display', 'crpstatus', $lCrpId);
      
          $lWebstatus = (isset($this->mRow[$this->mColKey])) ? $this->mRow[$this->mColKey] : '';
          $lCell = $lCrpStatusDisplay[$lWebstatus];
          $lRow .= '"' . utf8_decode($lCell) . '";"' . utf8_decode($lCrpStageName[$lWebstatus]) . '";';
  ...
  private function getViaPDB($aConfig) {
    ...
    if(!array_key_exists('jobnr', $lRequest->mCols)){
    ...
    while ($lPage * $lLpp <= $lMaxLines) {
      ...
      $lResponse = $lRequest -> getArray('jobnr');
