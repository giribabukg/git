 ...
 * @version $Rev: 15014 $
 * @date $Date: 2016-07-11 15:51:55 +0200 (Mon, 11 Jul 2016) $
...
class CInc_Api_Mail_Item extends CCor_Obj {
   ...
   */
  public function __construct($aFrom, $aFromName, $aTo, $aToName, $aSubject = '', $aText = '', $aHisInsertId = 0, $aEmailSend=NULL) {
    // START #22795 "From and reply-to in email headers editable"
    $lUsr = CCor_Usr::getInstance();
    $this -> mHisId = $aHisInsertId;
    $this -> mEmailSend = $aEmailSend;
    $lUid = $lUsr -> getId();
  ...
  public function insert($aSecrets = '', $aRealMID = NULL) {
    ...
    if (!empty($this -> mTo)) {
      ...
      if(isset($this -> mSentTo) AND !empty($this -> mSentTo)){
      	$lRes = $this -> mSentTo;
      }else{
      	$lRes = $this -> mBody;
      }
      ...
      if (CCor_Cfg::get('smtp.disabled', FALSE) OR ($this -> mEmailSend !='y' &&  !empty($this -> mEmailSend))) {
  ...
  public function setSentTo($aSent) {
    $this -> mSentTo = $aSent;
  }
