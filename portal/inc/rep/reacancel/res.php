<?php
class CInc_Rep_Reacancel_Res extends CRep_Base_Res {
 ...
 public function __construct($aSrc) {
  ...
  public function getResult() {
   ...
   $this -> addColumns();
   $this->addCondition('cancel', '!=', '');
   $lRes = $this -> mIte -> getArray();
   
   foreach($lRes as $lIdx => $lEntry) {
   	$lJid = $lEntry['jobid'];
   		$lSqlHistory = "SELECT datum, msg FROM al_job_his WHERE src_id = '".$lJid."' AND subject = 'Cancel Job'";
