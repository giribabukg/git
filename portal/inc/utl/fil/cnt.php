...
class CInc_Utl_Fil_Cnt extends CCor_Cnt {
  ...
  protected function getCloudflow() {
    $lFn = $this->getReq('fn');
    //http://cloudflow.5flow.net:9090/portal.cgi?asset=download_file&url=cloudflow%3A%2F%2FPP_FILE_STORE%2Fhybridtest%2FDose.pdf
    $lApi = new CApi_Cloudflow_Client();
    $lApi->download($lFn);
    exit;
  }

  protected function actViewproofscope() {
    $lFile = $this->getReq('file');
    //echo $lFile.'hi'; exit;
    $lApi = new CApi_Cloudflow_Client();
    $lUrl = $lApi->getSingleViewerUrl($lFile);
    if ($lUrl) {
      //echo $lUrl;
      $this->redirect($lUrl);
    }
  ...
  protected function actViewproofscopediff() {
    $lPref = 'cloudflow://';
    $lFile = $this->getReq('file');
    $lDiff = $this->getReq('diff');
    $lApi = new CApi_Cloudflow_Client();
    $lUrl = $lApi->getDiffViewerUrl($lPref.$lFile, $lPref.$lDiff);
    if ($lUrl) {
      $lRet['status'] = 'ok';
      $lRet['url'] = $lUrl;
    } else {
      $lRet['status'] = 'error';
      $lRet['message'] = 'Error: please select 2 files';
    }
