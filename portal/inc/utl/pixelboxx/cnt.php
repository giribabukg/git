...
class CInc_Utl_Pixelboxx_Cnt extends CCor_Cnt {
  ...
  protected function actDownload() {
    $lDoi = $this->getReq('doi');
    $lType = $this->getReq('type', 'fine');
    $lQry = new CApi_Pixelboxx_Query_Getobject();
    $lRes = $lQry->download($lDoi, $lType);
    ...
    $lCt = $lRes['content_type'];
    //echo $lCt;exit;
    if ($lCt == 'image/jpeg') $lNam.='.jpg';
    if ($lCt == 'image/png') $lNam.='.png';
    //header('Content-Type: application/octet-stream');
    header('Content-Type: '.$lRes['content_type']);
