...
class CInc_Usg_Opt_Cnt extends CCor_Cnt {
  ...
  protected function actPwd() {
    ...
    if (empty($lPwd)) {
      $this -> msg('Password is empty, please check the password configuration variable!', mtUser, mlError);
      $this -> redirect('index.php?act=usg-opt&id="'.$lUid.'"');
    }
    ...
    $lTpl -> setPat('date', $lDate);
    $lTpl -> setPat('link', CCor_Cfg::get('base.url'));
    ...
    // update
    $lQry = new CCor_Qry();
    $lQry -> query('UPDATE al_usr SET pass="'.$lPwd.'", lastreset_password= "'.$lCurrentTime.'", password_disable= "Y", login_attempts=0, pass_req="N" WHERE id='.$lUid);
    //Update to pass history
    CApp_Pwd::addedToPassHis($lUid, $lPwd);
