<?php
class CInc_Arc_Fil_Form extends CHtm_Form {

  public function __construct($aSrc, $aJobId, $aSubpath = '', $aDiv = '') {
    parent::__construct('arc-fil.supload', lan('lib.upload').' '.lan('job-'.$aSrc.'.menu').' '.lan('lib.file').' '.jid($aJobId, TRUE));
  ...
  protected function getCatArray() {
    $lRet = array();
    $lQry = new CCor_Qry('SELECT id,value_'.LAN.' AS value_lan FROM al_htb_itm WHERE mand IN ('.MID.',0) AND domain=\'fil\' ORDER BY value;');
    foreach ($lQry as $lRow) {
      $lRet[$lRow -> id] = $lRow -> value_lan;
    }
    return $lRet;
  }
  ...
  protected function getButtons() {
    ...
    if (!empty($this -> mCancel)) {
      $lRet.= btn(lan('lib.cancel'), "Flow.Std.ajxUpd(".$lParamsJSONEnc.")", 'img/ico/16/cancel.gif');
    }
    $lRet.= '</div>'.LF;
    return $lRet;
  }
