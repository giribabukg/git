...
class CInc_Htm_Fie_Fac extends CCor_Obj {
  ...
  protected function getTypeImage($aVal, $aState) {
    ...
    if (empty($aVal)) {
      ...
      if (file_exists($lDynamics['thumbnail_dir'].$lDynamics['thumbnail_file'])) {
        $aVal = $lDynamics['thumbnail_dir'].$lDynamics['thumbnail_file'];
      } else {
        $aVal = $lStatics['thumbnail_not_found'];
      }

      if (file_exists($lDynamics['image_dir'].$lDynamics['image_file'])) {
        $aValLarge = $lDynamics['image_dir'].$lDynamics['image_file'];
        $lImg = '<img src="'.$lDynamics['image_dir'].$lDynamics['image_file'].'" border="0px" width="300px" height="300px" alt="" />';
      } else {
        $aValLarge = $lStatics['image_not_found'];
        $lImg = '<img src="'.$lStatics['image_not_found'].'" border="0px" width="300px" height="300px" alt="" />';
      }
    }
  ...
  protected function getTypeUselect($aVal, $aState) {
    ...
    if (!empty($lParent)) {
      ...
      $lJs = 'document.observe("dom:loaded", function () {'.LF;
  ...
  protected function getTypeGselect($aVal, $aState) {
    ...
    */
    ...
    $lPageJs = 'document.observe("dom:loaded", function () {'.LF;
  ...
  protected function getTypeCselect($aVal, $aState) {
    ...
    $lId = $this->getId();
    $lJs = 'document.observe("dom:loaded", function () {'.LF; //dom:loaded is executed before jQuery ready event
