...
class CInc_Sys_Mail_List extends CHtm_List {
  ...
  public function __construct() {
    ...
    $this -> addColumn('mail_date', lan('lib.file.date'), TRUE);
    ...
    if ($this -> mCanDelete) {
      $this -> addDel();
      $this -> addBtn(lan('lib.del.all'), 'javascript:Flow.Std.cnfDel("index.php?act=sys_mail.delall", "'.LAN.'")', '<i class="ico-w16 ico-w16-del"></i>');
    }
    $this -> getPrefs();
