...
class CInc_Job_Fil_Files extends CHtm_List {
  ...
  */
  ...
  protected function getFlinkButton() {
    ...
    $lRet.= '<td align="left">';
    $lRet.= '    <div class="dn" id="flink_files_select" title="'.lan('lib.file.select.dialog.title').'">';
    $lRet.= '        <table>';
    ...
    $lRet.= '                    <td class="th2 nw w16" id="date">'.lan('lib.file.date').'</td>';
    $lRet.= '                    <td class="th2 nw w100p" id="edit">'.lan('lib.file.edit').'</td>';
  ...
  protected function getTdCategory() {
    ...
    if ($lVal > -1 && isset($this -> mCategory[$lVal])) {
      ...
      $lCat = lan('lib.file.cat.name.missing');
    }
    ...
    if ($this -> mUsr -> canEdit('job-fil')) {
      ...
      $lRet = '<div class="dn" id="category-change" title="'.lan('lib.file.cat.dialog.title').'">';
