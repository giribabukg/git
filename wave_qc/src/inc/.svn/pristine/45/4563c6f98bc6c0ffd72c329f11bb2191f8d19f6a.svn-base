<?php
class CInc_Job_Adm_Cos_Cnt extends CCor_Cnt {
  ...
  public function actStd() {
    ...
    $lVie = new CJob_Adm_Tabs($lJid, 'cos');
    $lRet.= $lVie -> getContent();

    $lFrm = new CJob_Adm_Cos_List($lJid, $lJob);
  ...
  protected function actScos() {
    ...
    $this -> redirect('index.php?act=job-adm-cos&jobid='.$lJid);
  }
