...
class CInc_Job_His_Mailform extends CHtm_Form {
  ...
  public function __construct($aSrc, $aJobId, $aJob, $aFrm = 'his', $aMailRequestId = 0) {
    $lUsr = CCor_Usr::getInstance();
    $lRedirectCancel = 'job-'.$aSrc.'-his&jobid='.$aJobId;
    if (!($lUsr -> canRead('job-his'))) {
      $lRedirectCancel = 'job-'.$aSrc.'.edt&jobid='.$aJobId;
    }

    parent::__construct('job-'.$aSrc.'-his.snewmail', 'New eMail', $lRedirectCancel);
    ...
    $this -> getTemplate(CCor_Cfg::get('job.notifytemplate', NULL));
  ...
  protected function getFieldForm() {
    ...
    if (!empty($this -> mPer)) {
      ...
      if ($this -> mMailRequestId > 0 AND $this -> mMailInfo['response'] == 1) {
        ...
        $lRet.= '<tr><td><input type="checkbox" name="val[uid][]" disabled="disabled" checked="checked" value="'.$this -> mMailInfo['sender_id'].'" /></td><td></td><td></td><td>'.$lSenderNameNeedResponse.'</td></tr>'.LF;
