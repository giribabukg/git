...
class CInc_Job_Fil_Pixelboxx_Details extends CCor_Tpl {
  ...
  protected function loadTemplate() {
    $lName = $this->getProjectFilename('pboxdetails.htm');
    $this->open($lName);
  }
  ...
  protected function fillMenu() {
    ...
    if ($lCart->isInCart($this->mDoi)) {
      $lMen->addJsItem('Flow.pixelboxx.removeFromCart('.$lPar.')', 'Remove from Cart', 'ico/16/check-lo.gif');
    } else {
      $lArr = array();
      $lArr['doi'] = $this->mDoi;
      ...
      $lMen->addJsItem('Flow.pixelboxx.addToCart(\''.htm($lEnc).'\','.$lPar.')', 'Add to Cart', 'ico/16/check-hi.gif');
    }
    $lMen->addItem('index.php?act=utl-pixelboxx.download&doi='.$this->mDoi, 'Download', 'ico/16/flag-04.gif');
    $lMen->addJsItem('Flow.pixelboxx.uploadNewVersion('.$lPar.')', 'Upload New Version', 'ico/16/folder.png');
    $lMen->addJsItem('Flow.pixelboxx.deleteFile('.$lPar.')', 'Delete File', 'ico/16/del.gif');
