<?php
class CInc_Job_Sec_Apl_Cnt extends CCor_Cnt {
  ...
  protected function actStd() {
    ...
    $lVie = new CJob_Sec_Tabs($lJid, 'apl');
    $lRet.= $lVie -> getContent();

    $lFrm = new CJob_Apl_List('sec', $lJid);
    $lRet.= $lFrm -> getContent();

    #$lApl = new CApp_Apl_Loop('sec', $lJid, 'apl');
    #$lApl -> createLoop('2008-07-01');

    $lFrm = new CJob_Sec_Apl_Form('job-sec-apl.sedt', $lJid, $lJob, $lPag);
    $lRet.= $lFrm -> getContent();
  ...
  protected function actNewmail() {
    ...
    $lVie = new CJob_Apl_Mailform('sec', $lJid, $lJob);
    $lRet.= $lVie -> getContent();
  ...
  protected function actSnewmail() {
    ...
    $lMod = new CJob_His_Mod('sec', $lJid, htMail);
   ...
   # $lMod -> insert(); //braucht nicht dokumentiert zu werden.
    ...
    $this -> redirect('index.php?act=job-sec-apl&jobid='.$lJid);
  }
