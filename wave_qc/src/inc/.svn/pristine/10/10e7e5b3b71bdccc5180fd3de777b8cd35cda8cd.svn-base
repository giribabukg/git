<?php
class CInc_Job_Sec_Cos_Cnt extends CCor_Cnt {
  ...
  public function actStd() {
    ...
    $lVie = new CJob_Sec_Tabs($lJid, 'cos');
    $lRet.= $lVie -> getContent();

    $lFrm = new CJob_Sec_Cos_List($lJid, $lJob);
  ...
  protected function actScos() {
    ...
    $this -> redirect('index.php?act=job-sec-cos&jobid='.$lJid);
  }
