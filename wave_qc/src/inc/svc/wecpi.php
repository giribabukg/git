...
//       }
      ...
      if ('portal' == $lWriter) {
        ...
        $lIte -> setLimit($lLimitFrom, $lStatics['fetch_number']);
        ...
        $lRequest -> setLimit($lLimitFrom, $lStatics['fetch_number']);
  ...
  protected function copy_to_networker($aClient) {
    ...
    foreach ($lQuery as $lRow) {
      ...
      $lImageFound = $lStatics['getcwd'].$lDynamics['image_dir'].$lDynamics['image_file'];

      $lQry = new CApi_Alink_Query('putFile');
      $lQry -> addParam('sid', $this -> mClients[$lClient]);
      $lQry -> addParam('jobid', $lJobID);
      $lQry -> addParam('filename', $lDynamics['image_file']);
  ...
  protected function copy_to_portal($aClient) {
    if (0 == $this -> mWecTns && 1 == $this -> mWecPi) {
      ...
      foreach ($lQuery as $lRow) {
        ...
        header('Content-Disposition: attachment; filename="'.$lDynamics['image_file'].'"');
        ...
        if (0 == $lErr) {
          ...
          if (!empty($lRet)) {
            $lFile = fopen($lPreviewImageFound.$lDynamics['image_file'], "w");
            fwrite($lFile, $lRet);
            fclose($lFile);
            ...
            $lImageWidth = imagesx($lImage);
              ...
              imagejpeg($lThumbnail, $lPreviewImageFound.$lDynamics['thumbnail_file'], 75);
            }
