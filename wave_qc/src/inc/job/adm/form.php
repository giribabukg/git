 ...
 */
class CInc_Job_Adm_Form extends CJob_Form {
  ...
  public function __construct($aAct, $aJobId = 0, $aJob = NULL, $aPage = 'job') {
    ...
    $this -> addBtn('act', lan('lib.cancel'), 'go("index.php?act=job-adm")', 'img/ico/16/cancel.gif', 'button', array('class' => 'btn w200' ));
    if (!empty($this -> mJobId)) {
      $this -> addBtn('act', lan('lib.print'), 'pop("index.php?act=job-adm.prn&jobid='.$this -> mJobId.'")', 'img/ico/16/print.gif', 'button', array('class' => 'btn w200' ));
    }

    if ($this -> canAssign()) {
      $this -> addBtn('act', lan('job.assignprj'), 'go("index.php?act=job-adm.assignprj&jobid='.$this -> mJobId.'")', 'img/ico/16/next-hi.gif', 'button', array('class' => 'btn w200' ));
    }
    ...
    if ($this -> canAssignSkuSub()) {
      $this -> addBtn('act', lan('job.assignskusub'), 'go("index.php?act=job-adm.assignskusub&jobid='.$this -> mJobId.'")', 'img/ico/16/next-hi.gif', 'button', array('class' => 'btn w200' ));
    }
    ...
    $this -> mTabs = new CJob_Adm_Tabs($this -> mJobId, $aPage);
    ...
    if (($lUsr -> canEdit('job-wec-id')) && ($this->mJobId) &&(substr($this->mJobId, 0, 1) != 'A')) {
      ...
      $lAtt = array();
      $lAtt['class'] = 'btn w200';
      $this -> addBtn('act', lan('wec.menu'), 'go("index.php?act=job-adm.wec&src='.$this -> mSrc.'&direct=1&jobid='.$this -> mJobId.'")', 'img/ico/16/wec.gif', 'button', $lAtt);
    }
