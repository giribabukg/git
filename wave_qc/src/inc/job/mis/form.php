...
class CInc_Job_Mis_Form extends CJob_Form {
  ...
  public function __construct($aAct, $aJobId = 0, $aJob = NULL, $aPage = 'job') {
    ...
    $this -> addBtn('act', lan('lib.cancel'), 'go("index.php?act=job-mis")', 'img/ico/16/cancel.gif', 'button', array('class' => 'btn w200' ));
    if (!empty($this -> mJobId)) {
      $this -> addBtn('act', lan('lib.print'), 'pop("index.php?act=job-mis.prn&jobid='.$this -> mJobId.'")', 'img/ico/16/print.gif', 'button', array('class' => 'btn w200' ));
    }
    if ($this -> canAssign()) {
      $this -> addBtn('act', lan('job.assignprj'), 'go("index.php?act=job-mis.assignprj&jobid='.$this -> mJobId.'")', 'img/ico/16/next-hi.gif', 'button', array('class' => 'btn w200' ));
    }
